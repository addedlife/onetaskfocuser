...
const AI_JOB_REGISTRY = {
  ...
  "dashboard.nervecenter_summary.v1": {
    ...
    schema: '{"supercrunch":"comma-separated line of specific named items from ALL sources ordered by urgency — NEVER counts or category nouns; name the actual thing","signals":[{"area":"Calendar","note":"terse Apple-notification-style note; name the things not the count"}]}',
    buildPrompt(input = {}) {
      const context = normalizeChiefContext(input.context || input);
      return compactLines([
        YESHIVISH_SYSTEM,
        "You are generating two display strings for a personal command-center dashboard. Be a world-class executive briefer.",
        "SUPERCRUNCH: one comma-separated line of the actual named items across ALL sources, ordered by urgency/consequence. Max ~200 chars; stop after the highest-priority items if there are too many — do NOT add counts or fillers at the end.",
        "Rules for supercrunch:",
        "  1. Name the specific thing. Task text, email topic/sender, shaila subject, caller name, event title. Never a category noun.",
        "  2. NEVER write a number or count anywhere in the supercrunch. Not '3 tasks'. Not '2 emails'. Not 'various 4 items'. If you feel like writing a number, write another item name instead.",
        "  3. Routine calendar items (regular davening, standard learning sessions) are background — omit unless they are the only data.",
        "  4. Wrong: '3 tasks, 2 emails, 1 shaila' | Right: 'Reuven callback, Chase overdraft, Mincha 4pm, Kashrus of the pot'",
        "  5. Wrong: 'Tasks: pending items, Emails: unread' | Right: 'Invoice due Friday, shipping delay, dentist prep'",
        "SIGNALS: one entry per area that has meaningful data (Calendar, Mail, Tasks, Shailos, Phone). Apple notification style — terse, telegraphic, lead with the most concrete fact. Drop articles and filler. Semicolons between items.",
        "Rules for signals:",
        "  1. Name the things, not the count. 'Reuven; Mom; unknown caller' not '3 missed calls'. 'Kashrus of the pot; oven question' not '2 shailos'.",
        "  2. Do NOT restate the area label — the UI already shows it. No 'Tasks:' prefix in a Tasks note.",
        "  3. For Tasks: lead with the most urgent. 'Reuven follow-up; invoice approval; shipping delay'.",
        "  4. For Calendar: lead with next/soonest non-routine event. 'Dentist 3pm; Mincha 4pm'.",
        "  5. For Mail: sender + topic. 'Chase: overdraft; Amazon: delivery update'.",
        "  6. For Phone: missed callers by name, THEN texts by sender + topic. 'Reuven: shiur tomorrow; Mom: dinner plans; missed: Dr. Cohen'. If only texts, omit the 'missed:' prefix.",
        "  7. Omit inactive areas entirely.",
        "Reliability: use ONLY facts in the snapshot. Do not invent, infer, or add advice.",
        `Current snapshot:\n${jsonBlock(context)}`,
        responseJsonInstruction("object", this.schema),
      ]);
    },
