...
function VoiceInput({ onResult, onClose, onAddShailos, onExistingShailaAnswers, existingShailos, color, T, aiOpts }) {
  ...
  const shailaParseBtn = (
    aiOpts ? (
      <button onClick={parseAsShailos} disabled={shailaLoading} style={{
        ...
        fontSize:NC_TYPE.meta, fontWeight:`var(--nc-fw-semibold, 600)`, background:"none",
  ...
  const shell = (brd) => ({
    ...
    animation: "ot-fade 0.2s", fontFamily: NC_FONT_STACK,
  });
  ...
  const applyBtn = (bg) => (
    <button onClick={() => applyText(editText)} disabled={!editText.trim()} style={{
      ...
      fontSize: NC_TYPE.body, fontWeight: `var(--nc-fw-strong, 700)`, fontFamily: NC_FONT_STACK,
  ...
  if (phase === "recording") return (
    <div style={shell(color)} data-voice-panel="true">
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
        <span style={{ fontSize:NC_TYPE.small, color:T.danger, fontWeight:`var(--nc-fw-strong, 800)`, letterSpacing:1.2 }}>🔴 LISTENING</span>
        <div style={{ display:"flex", gap:8, alignItems:"center" }}>
          {aiOpts && (
            <button onClick={() => { shailaAutoFiredRef.current = false; setShailaMode(true); stopRec(); }} title="Stop and parse as shailos" style={{
              ...
              fontFamily:NC_FONT_STACK, fontWeight:`var(--nc-fw-strong, 700)`, lineHeight:1,
          ...
          {closeBtn}
          <button onClick={stopRec} style={{ background:T.danger, color:"#fff", border:"none", borderRadius:7, padding:"5px 16px", fontSize:NC_TYPE.small, cursor:"pointer", fontWeight:`var(--nc-fw-strong, 700)` }}>STOP</button>
        </div>
      </div>
      <div style={{
  ...
  if (phase === "gemini_wait") return (
    <div style={shell(color)} data-voice-panel="true">
      <div style={{ textAlign:"center", padding:"18px 0 20px" }}>
        ...
        <p style={{ margin:0, fontSize:NC_TYPE.body, color:T.tSoft, fontWeight:`var(--nc-fw-semibold, 600)`, fontFamily:NC_FONT_STACK }}>{geminiStatus || "Transcribing..."}</p>
    ...
    <div style={{ marginTop:6 }}>
      ...
      {detectedAnswers.filter(x => x.approved !== false).map((match, i) => (
        <div key={match.id || i} style={{ background:"#C8A84C0E", borderRadius:8, padding:"8px 10px", borderLeft:"3px solid #C8A84C", marginTop:4 }}>
          <div style={{ fontSize:NC_TYPE.small, fontWeight:`var(--nc-fw-strong, 700)`, color:"#C8A84C", fontFamily:NC_FONT_STACK, marginBottom:2 }}>Answers existing shailo:</div>
          ...
          <div style={{ display:"flex", gap:6 }}>
            ...
            }} style={{ flex:1, padding:"4px 8px", borderRadius:6, border:"none", background:"#C8A84C", color:"#fff", cursor:"pointer", fontSize:NC_TYPE.small, fontWeight:`var(--nc-fw-strong, 700)`, fontFamily:NC_FONT_STACK }}>✓ Save answer</button>
  ...
  if (phase === "reviewing") return (
    <div style={shell(T.brd)} data-voice-panel="true">
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
        <span style={{ fontSize:NC_TYPE.small, color:T.tFaint, fontWeight:`var(--nc-fw-strong, 700)`, letterSpacing:1.2, textTransform:"uppercase" }}>
  ...
  if (phase === "shaila_review") return (
    <div style={{...shell("#C8A84C"), maxHeight:"80vh", display:"flex", flexDirection:"column"}} data-voice-panel="true">
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10, flexShrink:0 }}>
        <span style={{ fontSize:NC_TYPE.small, color:"#C8A84C", fontWeight:`var(--nc-fw-strong, 700)`, letterSpacing:1 }}>✡ {parsedShailas.length} SHAILO{parsedShailas.length!==1?"S":""} DETECTED</span>
        {closeBtn}
      </div>
      <div style={{ flex:1, overflowY:"auto", display:"flex", flexDirection:"column", gap:10, marginBottom:10 }}>
        ...
        {detectedAnswers.length > 0 && (
          <div style={{ background:"#C8A84C14", borderRadius:10, border:"1px solid #C8A84C50", padding:"8px 10px", display:"flex", flexDirection:"column", gap:8 }}>
            <div style={{ fontSize:NC_TYPE.small, fontWeight:`var(--nc-fw-strong, 800)`, color:"#C8A84C", letterSpacing:1.2, marginBottom:2 }}>POSSIBLE ANSWERS TO EXISTING SHAILOS</div>
            {detectedAnswers.filter(x => x.approved !== false).map((match, i) => (
              <div key={match.id || i} style={{ background:T.bgW, borderRadius:8, padding:"8px 10px", borderLeft:"3px solid #C8A84C" }}>
                ...
                <div style={{ fontSize:NC_TYPE.small, fontWeight:`var(--nc-fw-strong, 700)`, color:"#C8A84C", letterSpacing:1, marginBottom:2 }}>ANSWER FOUND:</div>
                <div style={{ fontSize:NC_TYPE.meta, fontFamily:NC_FONT_STACK, color:T.tSoft, marginBottom:8, lineHeight:1.4 }}>{match.answer}</div>
                <div style={{ display:"flex", gap:6 }}>
                  <button onClick={() => {
                    if (onExistingShailaAnswers) onExistingShailaAnswers(match.id, match.answer);
                    setDetectedAnswers(p => p.map((x, j) => j===i ? {...x, approved:false} : x));
                  }} style={{ flex:1, padding:"5px 8px", borderRadius:7, border:"none", background:"#C8A84C", color:"#fff", cursor:"pointer", fontSize:NC_TYPE.small, fontWeight:`var(--nc-fw-strong, 700)`, fontFamily:NC_FONT_STACK }}>✓ Yes, save this answer</button>
                  <button onClick={() => setDetectedAnswers(p => p.map((x, j) => j===i ? {...x, approved:false} : x))}
          ...
          <div key={item.id} style={{ background:T.bgW, borderRadius:10, padding:"10px 12px", borderLeft:"3px solid #C8A84C" }}>
            ...
            {item.answer !== null && (
              <div style={{ borderTop:`1px solid ${T.brd}`, marginTop:6, paddingTop:6 }}>
                <div style={{ fontSize:NC_TYPE.small, color:"#C8A84C", fontWeight:`var(--nc-fw-strong, 800)`, letterSpacing:1.2, marginBottom:3 }}>ANSWER</div>
                <textarea
                  value={item.answer || ""}
            ...
            <div style={{ display:"flex", gap:8, marginTop:6 }}>
              <div style={{ flex:1 }}>
                <div style={{ fontSize:NC_TYPE.small, color:T.tFaint, fontWeight:`var(--nc-fw-strong, 700)`, letterSpacing:1, marginBottom:2 }}>ASKED BY</div>
              ...
              <div style={{ flex:1 }}>
                <div style={{ fontSize:NC_TYPE.small, color:T.tFaint, fontWeight:`var(--nc-fw-strong, 700)`, letterSpacing:1, marginBottom:2 }}>ANSWERED BY</div>
      ...
      <div style={{ flexShrink:0, display:"flex", flexDirection:"column", gap:6 }}>
        <button onClick={() => { if (onAddShailos) onAddShailos(parsedShailas); cleanup(); onClose(); }} style={{
          width:"100%", padding:"10px", fontSize:NC_TYPE.body, fontWeight:`var(--nc-fw-strong, 700)`,
