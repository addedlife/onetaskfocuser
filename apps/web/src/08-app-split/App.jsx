                  ...
                  <div key={rec.id} style={{background:C.bgSoft,border:`1px solid ${C.divider}`,borderRadius:RADIUS.sm,padding:10}}>
                    <div style={{display:"flex",justifyContent:"space-between",gap:8,alignItems:"flex-start"}}>
                      ...
                      <div style={{display:"flex",gap:4,flexShrink:0}}>
                        ...
                        <IconBtn icon="download" iconSize={18} color={C.muted}
                          onClick={()=>downloadHeldRecording(rec)} aria-label="Download audio file" title="Download audio"/>
                        <IconBtn icon="delete" iconSize={18} color={C.faint}
        ...
        {tab === "queue" && (
          <div style={{...commandPageWidth,animation:"ot-fade 0.3s",marginTop:24,flex:1}}>
            ...
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
              ...
              <IconBtn variant="outlined" icon={qCondensed ? "view_agenda" : "reorder"} iconSize={15} color={qCondensed ? C.accent : C.muted}
              ...
              <IconBtn variant="outlined" title="Settings" aria-label="Settings"
            ...
            <div style={{marginBottom:14,background:C.bg,border:`1px solid ${C.divider}`,borderRadius:RADIUS.md,padding:"8px 10px",display:"flex",flexDirection:"column",gap:8}}>
              ...
              {qAddPri && (
                <form onSubmit={e=>{e.preventDefault();const t=qAddText.trim();if(!t)return;const newQT={id:uid(),text:t,completed:false,priority:qAddPri,createdAt:Date.now()};if(qAddPin)newQT.pinned=true;uT(ts=>doOpt(qAddPin?[newQT,...ts]:[...ts,newQT]));setQAddText("");setQAddPri(null);setQAddPin(false);clearTimeout(queueToastTmr.current);const clr=gP(pris,newQT.priority).isShaila?"#C8A84C":gP(pris,newQT.priority).color;setQueueToast(clr);setQueueToastKey(k=>k+1);queueToastTmr.current=setTimeout(()=>setQueueToast(null),5000);triggerAIPrioritize();}} style={{display:"flex",gap:6,animation:"ot-fade 0.2s",minWidth:0}}>
                  ...
                  <IconBtn variant="filled" type="submit" title="Add task" aria-label="Add task"
