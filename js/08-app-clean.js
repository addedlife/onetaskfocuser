...
function App({ user, onSignOut }) {
  ...
  const [chgPri, setChgPri] = useState(null);
  ...
  useEffect(() => {
    ...
    Store.ls(AS);  // Refresh optional offline cache
    clearTimeout(saveTmr.current);
    saveTmr.current = setTimeout(() => Store.saveToFB(AS), 1500); // debounced Firebase write
  ...
  useEffect(() => {
    ...
    const { tasks: aged, anyChanged } = applyTaskAging(AS.lists[0]?.tasks || [], pris);
    if (!anyChanged) return;
    const count = aged.filter(t => t.autoAged && !AS.lists[0].tasks.find(o => o.id===t.id && o.autoAged)).length;
    uT(() => aged);
  ...
  useEffect(() => {
    ...
    const unsub = ref.onSnapshot(snap => {
      ...
      setAS(prev => {
        if (!prev) return prev;
        const localTs = prev._lsModified || 0;
        // Adopt if Firebase has genuinely newer data. Echo-protection is automatic:
        // our own saves set localTs = fbTs, so the '>' check blocks self-echo cleanly.
        if (fbTs > localTs) {
          Store.ls(fbState);
          return fbState;
        }
  ...
  useEffect(() => {
    if (!loaded || !AS) return;
    const flush = () => { Store.flushSync(AS); };
    window.addEventListener("beforeunload", flush);
    window.addEventListener("pagehide", flush);
    // Periodic Firebase sync — only if local state is newer than what we last saved
    const iv = setInterval(() => {
      if (loaded && AS && (AS._lsModified || 0) > Store._lastSavedFbModified) Store.saveToFB(AS);
    }, 15000);
    return () => {
  ...
  // ─── Blocked task resume nudge (F8) ──────────────────────────────────────
  useEffect(() => {
    actT.filter(t => t.blocked && t.blockedAt).forEach(t => {
      if (blockedTmr.current[t.id]) return; // already scheduled
      const elapsed = Date.now() - t.blockedAt;
      const remaining = Math.max(0, 3 * 3600000 - elapsed);
  ...
  // ─── Helpers ─────────────────────────────────────────────────────────────
  function uT(fn) { setAS(p => ({...p, _lsModified: Date.now(), lists: p.lists.map(l => l.id === p.activeListId ? {...l, tasks: fn(l.tasks)} : l)})); }
  ...
  async function launchpadOptimize() {
    if (!effectiveGK || optLoading) return;
    setOptLoading(true);
    try {
      const {optimized, alreadyOptimal, insight, pinOverride} = await aiOptTasksWithAnalysis(tasksRef.current, pris, effectiveGK);
      if (pinOverride) {
  ...
  async function captureZenDump(text) {
    ...
    if (!effectiveGK) {
      const tasks = text.split('\n').filter(l=>l.trim()).map(l=>({id:uid(),text:l.trim(),priority:lowestPri}));
      setZenDumpParsed(p=>[...p,...tasks]);
      return;
    }
    setZenDumpParsing(true);
    try {
      const parsed = await aiParseBrainDump(text, pris, effectiveGK);
  ...
  function legacyCompTask(id) { compTask(id, false, true); }
  ...
  function saveShailaField(id, field, value) {
    setAS(p => ({...p, _lsModified: Date.now(), lists: p.lists.map(l => ({...l, tasks: l.tasks.map(t => t.id===id ? {...t, [field]: value} : t)}))}));
  }
  ...
  function addShailas(items) {
    ...
    setAS(p => ({...p, _lsModified: Date.now(), lists: p.lists.map(l =>
      l.id === p.activeList ? {...l, tasks: [...l.tasks, ...newTasks]} : l
    )}));
    setShowVoice(false);
  }
  ...
  async function openFirstStep(task) {
    ...
    try {
      const step = await suggestFirstStep(task.text, effectiveGK);
  ...
  function blockTask(id, note) {
    ...
    uT(ts => {
      ...
      const blocked = {...task, blocked:true, blockedAt:Date.now(), blockedNote:note, priority:task._origPriority||task.priority, _origPriority:task.priority};
  ...
  async function manOpt() {
    if (effectiveGK && actT.length >= 3) {
      setOptLoading(true);
      const optimized = await aiOptTasks(tasks, pris, effectiveGK);
      uT(() => optimized);
      setOptLoading(false);
    } else {
      uT(ts => doOpt(ts));
  ...
  function chgPriority(tid, np) {
    uT(ts => {
      const task = ts.find(t => t.id === tid);
      if (task?.parentTask) {
        // Change priority for all tasks in the group
        return doOpt(ts.map(t => t.parentTask === task.parentTask ? {...t, priority:np} : t));
      }
...
TAKEAWAY: [The single most useful thing to know. One sentence. Specific.]`;
    const result = await callGemini(effectiveGK, prompt);
  ...
  const sendAiChat = async (msg) => {
    if (!effectiveGK || !metrics || !msg.trim()) return;
...
Give a thorough, analytical response (4-8 sentences) with specific numbers and actionable insights. No bullet points or headers.`;
    const result = await callGemini(effectiveGK, sysPrompt);
    setAiChatHistory(h => {
  ...
  const switchTab = (t) => { setTab(t); setShowLM(false); setNavExp(false); setShowQueueMenu(false); setShowAides(false); setShowEntryTools(false); };
  ...
  return (
    <div ref={appRef} style={{height:"100vh",overflow:"hidden",background:`linear-gradient(170deg,${T.grad[0]} 0%,${T.grad[1]} 50%,${T.grad[2]} 100%)`,fontFamily:"Georgia,'Palatino Linotype','Book Antiqua',serif",color:T.text,display:"flex",flexDirection:"column",alignItems:"center"}}>
      ...
      {showBD !== null && <TaskBD task={showBD===true?null:showBD} pris={pris} T={T} onConfirm={confirmBD} onClose={()=>setShowBD(null)} gk={effectiveGK}/>}
      ...
      {blockedResume && actT.find(t=>t.id===blockedResume) && (
        <div style={{position:"fixed",bottom:80,left:"50%",transform:"translateX(-50%)",background:T.card,border:`1.5px solid ${T.brd}`,borderRadius:14,padding:"12px 16px",boxShadow:T.shadowLg,zIndex:9500,maxWidth:340,width:"90%",animation:"ot-fade 0.3s"}}>
          ...
          <div style={{display:"flex",gap:8}}>
            <button onClick={()=>{
              // Snooze: reset blockedAt to now so 3hr timer restarts
              uT(ts=>ts.map(t=>t.id===blockedResume?{...t,blockedAt:Date.now()}:t));
              // Clear any scheduled timer so useEffect reschedules cleanly
      ...
      {chgPri && (
        <div style={{position:"fixed",inset:0,zIndex:9000,background:"rgba(0,0,0,0.3)",display:"flex",alignItems:"center",justifyContent:"center"}} onClick={()=>setChgPri(null)}>
          <div onClick={e=>e.stopPropagation()} style={{background:T.card,borderRadius:18,padding:"20px 24px",boxShadow:T.shadowLg}}>
            <p style={{fontSize:13,fontWeight:600,margin:"0 0 14px",fontFamily:"system-ui"}}>Change priority:</p>
            <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
              {ap.map(p => (
                <button key={p.id} onClick={()=>chgPriority(chgPri,p.id)} style={{display:"flex",alignItems:"center",gap:6,padding:"8px 14px",borderRadius:10,border:`2px solid ${p.color}`,background:pBg(p.color),cursor:"pointer",fontSize:12,fontWeight:600,fontFamily:"system-ui",color:T.text}}>
      ...
      {showBlockReflect && curT && (
        <BlockReflectModal task={curT} T={T} gk={effectiveGK} onClose={()=>setShowBlockReflect(false)}/>
      )}

      {/* ShailaManager */}
      {showShailaManager && (
        <ShailaManager AS={AS} T={T} gk={effectiveGK} onSaveField={saveShailaField} onClose={()=>setShowShailaManager(false)}/>
      )}
      ...
      <div style={{width:"100%",maxWidth:"min(800px, 95vw)",padding:"0 clamp(16px,3vw,32px)",position:"relative",zIndex:1,height:"100vh",overflowY:tab==="focus"?"hidden":"auto",display:"flex",flexDirection:"column"}}>
        ...
        {tab === "focus" && (
          <div style={{animation:"ot-fade 0.3s",display:"flex",alignItems:"center",justifyContent:"center",height:"100vh",overflow:"hidden"}}>
            ...
            {(()=>{
              ...
              const leftBot = [
                {icon: optLoading ? spinnerIcon : <IC.Sparkle s={16} c={T.tSoft}/>, label: optLoading ? "Thinking…" : effectiveGK ? "AI Prioritize" : "Prioritize", action: launchpadOptimize},
          ...
          <div style={{animation:"ot-fade 0.3s",marginTop:24,flex:1}}>
            ...
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
              ...
              <button
                ...
                disabled={optLoading}
                title={effectiveGK ? "AI Prioritize queue" : "Prioritize queue"}
              ...
              <div style={{position:"relative"}}>
                <button
                  onClick={()=>setShowQueueMenu(p=>!p)}
                  style={{width:32,height:32,borderRadius:10,border:`1px solid ${T.brd}`,background:T.bgW,cursor:"pointer",fontSize:16,color:T.tSoft,display:"flex",alignItems:"center",justifyContent:"center",lineHeight:1}}
                  title="More options"
                >⋮</button>
                {showQueueMenu && (
                  <div style={{position:"absolute",top:"100%",right:0,marginTop:4,background:T.card,borderRadius:14,border:`1px solid ${T.brd}`,boxShadow:"0 8px 32px rgba(0,0,0,0.12)",minWidth:200,zIndex:20,animation:"ot-fade 0.15s",overflow:"hidden"}}
                    onMouseLeave={()=>setShowQueueMenu(false)}>
                    {/* Energy filter */}
                    <div style={{padding:"10px 14px 6px",borderBottom:`1px solid ${T.brdS}`}}>
                      <div style={{fontSize:11,color:T.tFaint,fontFamily:"system-ui",fontWeight:700,letterSpacing:1,marginBottom:7,textTransform:"uppercase"}}>Energy filter</div>
                      <div style={{display:"flex",gap:5}}>
                        {["high","low",null].map(e => (
                          <button key={String(e)} onClick={()=>{setAS(p=>({...p,currentEnergy:e}));setShowQueueMenu(false);}} style={{padding:"4px 10px",borderRadius:8,border:`1px solid ${e==="high"?"#E07040":e==="low"?"#7EB0DE":T.brd}`,background:curEnergy===e?(e==="high"?"#E0704020":e==="low"?"#7EB0DE20":T.bgW):T.bgW,cursor:"pointer",fontSize:10,fontFamily:"system-ui",color:e==="high"?"#B85030":e==="low"?"#4A7898":T.tSoft}}>
                    ...
                    {/* Actions */}
                    <button onClick={()=>{manOpt();setShowQueueMenu(false);}} disabled={optLoading} style={{width:"100%",padding:"11px 14px",border:"none",background:"transparent",cursor:"pointer",fontSize:12,fontFamily:"system-ui",color:T.tSoft,display:"flex",alignItems:"center",gap:8,borderBottom:`1px solid ${T.brdS}`}}>
                      <IC.Sparkle s={13} c={T.tSoft}/> {optLoading?"Optimizing...":"Optimize order"}{effectiveGK&&!optLoading&&<span style={{fontSize:11,opacity:.5,marginLeft:2}}>AI</span>}
                    </button>
                    <button onClick={()=>{setShowBulk(true);setShowQueueMenu(false);}} style={{width:"100%",padding:"11px 14px",border:"none",background:"transparent",cursor:"pointer",fontSize:12,fontFamily:"system-ui",color:T.tSoft,display:"flex",alignItems:"center",gap:8,borderBottom:`1px solid ${T.brdS}`}}>
                      <IC.Bulk s={13} c={T.tSoft}/> Bulk add tasks
                    </button>
                    <button onClick={()=>{setShowBD(true);setShowQueueMenu(false);}} style={{width:"100%",padding:"11px 14px",border:"none",background:"transparent",cursor:"pointer",fontSize:12,fontFamily:"system-ui",color:T.tSoft,display:"flex",alignItems:"center",gap:8,borderBottom:`1px solid ${T.brdS}`}}>
                      <IC.Split s={13} c={T.tSoft}/> Shatter
                    </button>
                    {effectiveCount > overwhelmThreshold && (
                      <button onClick={()=>{setFocusModeActive(f=>!f);setShowQueueMenu(false);}} style={{width:"100%",padding:"11px 14px",border:"none",background:"transparent",cursor:"pointer",fontSize:12,fontFamily:"system-ui",color:"#C08830",display:"flex",alignItems:"center",gap:8}}>
                        😶 {focusModeActive ? `Exit focus mode (show all ${effectiveCount})` : `Focus mode (${effectiveCount} → ${overwhelmThreshold})`}
                      </button>
                    )}
                  </div>
                )}
                      ...
                      return (
                        <div key={task.id} draggable onDragStart={()=>setDragId(task.id)} onDragOver={e=>e.preventDefault()} onDrop={()=>handleDrop(task.id)} style={{display:"flex",alignItems:"center",gap:6,padding:"12px 10px",borderBottom:`1px solid ${T.brdS}`,borderLeft:`3px solid ${tp.color}`,background:isF?pBg(tp.color):(task.blocked?pBg("#E0B472"):"transparent"),cursor:"grab",opacity:task.blocked?.6:1}}>
                          ...
                          <div draggable={false} onPointerDown={e=>e.stopPropagation()} style={{display:"flex",gap:0,flexShrink:0}}>
                            ...
                            <button onClick={e=>{e.stopPropagation();openFirstStep(task);}} title="Suggest first step with AI" style={{background:"none",border:"none",cursor:effectiveGK?"pointer":"default",padding:3,opacity:effectiveGK?.35:.15,fontSize:11}} onMouseEnter={e=>{if(effectiveGK)e.currentTarget.style.opacity=1;}} onMouseLeave={e=>e.currentTarget.style.opacity=effectiveGK?.35:.15}>✦→</button>
        ...
        {tab === "insights" && (
          <div style={{animation:"ot-fade 0.3s",marginTop:24}}>
            ...
            {/* ── AI Insight card (requires Gemini key) ── */}
            {effectiveGK && (
