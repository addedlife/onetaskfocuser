...
function SubtaskGroup({parentTask, tasks, pris, T, onMoveTop, onComp, onDel, onEdit, onAdd, onReorder, onChgPri, searchQ}) {
      ...
      {open && (
        <div style={{padding:"6px 0",animation:"ot-fade 0.15s"}}>
          {steps.map((st, i) => {
            ...
            return (
              <div key={st.id}
                ...
                <button onClick={e=>{e.stopPropagation(); if(e.altKey) legacyCompTask(st.id); else onComp(st.id);}} style={{width:18,height:18,borderRadius:"50%",border:`1.5px solid ${pri.color}`,background:"transparent",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}} title="Complete (Alt+Click for Legacy)">
          ...
          <div style={{display:"flex",gap:6,padding:"6px 12px 4px"}}>
            ...
            <button onClick={e=>{if(e.altKey) steps.forEach(s=>legacyCompTask(s.id)); else steps.forEach(s=>onComp(s.id));}} style={{flex:1,padding:"5px 8px",borderRadius:7,border:"none",background:pri.color+"20",cursor:"pointer",fontSize:10,fontWeight:600,fontFamily:"system-ui",color:pri.color}}>✓ All done</button>
          </div>
        </div>
      )}
    </div>
