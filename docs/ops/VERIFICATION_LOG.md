...
- Owner report (tablet browser): "Can't reach the phone link" error showing while the feed was live via the PC host. Root cause in `NerveCenterPhoneSurface.jsx`: only a successful REST relay fetch cleared the error banner â€” data arriving via the Firestore `phone-relay/state` listener never did. A few failed fetches at mount (before the listener's first server snapshot) set the banner, then the listener carried the whole session and the stale error sat over a live feed forever.
- Fix: the listener now counts as transport success (resets `failStreakRef`, stamps `lastFetchOkAtRef`, clears the error). New `errorIsTransportRef` distinguishes transport errors (listener may clear) from command failures (never auto-cleared â€” the user must see a command didn't run). Error copy made host-neutral ("make sure a phone host (tablet or PC) is running" instead of blaming the tablet while the PC hosts).
- Switch UX (owner spec, same message): while a host handover is pending, the SOLID highlight on the rail's Tablet|PC segmented control stays on the ACTUAL holder (`owner.host`) and the REQUESTED host's segment blinks (`nc-host-blink` keyframe in `ui-tokens.jsx`) until the new host's heartbeat confirms â€” then solid moves, blink stops. Same treatment: collapsed-rail host icon, the phone card's status dot (`link.switching`), and the DeskPhone web page dot (`.dp-status-dot.is-switching`). All driven by the shared `phone-link.js` machine's existing `switching` state â€” no new data plumbing, no host rebuilds.
- Takeover-compliance ask ("all sources recognize an authorized switch"): already shipped in 4.40.127 / DeskPhone b331 / Android b7 â€” no code change; requires both devices to run those builds.
...
- Gates: `npm run build` (apps/web) â†’ 0 errors, pushed `f4121e5c`; deploy Action green (Deploy to Firebase run 29200539172).
...
## 2026-07-06 GM3 follow-up â€” card-grid tonal spacing, column-view header reclaim, mobile bottom-cut fix (4.34.117)

- Owner feedback on 4.34.116, three items, all in NerveCenterNext:
- **Card-grid ("compact display") GM3 spacing**: `MobileBox`/`MobileSection` cards are now borderless plain-surface (C.bg, radius 16, no 1px outline) on the deeper tonal page (`pageBg`) â€” the same tone-and-space language as the column view; grid gutters widened 3/5px â†’ 8 (dense) / 14 (comfortable); stacked view gets the same page/card/gutter treatment. Scroll-fade + status-dot ring colors updated to match the new card surface.
- **Column-view header reclaim** (the row the owner actually meant): the Boxes/Full + density + Full/Focus control row is lifted out of the layout flow â€” absolutely positioned in the page's top-right margin (fixed root as containing block), inner container `paddingTop: 38` reserves its lane. Net ~30â€“40px returned to the panes; controls remain one-touch.
...
- Gates: `npm run build` (apps/web) â†’ 0 errors. BUILD-VERIFIED, pushed live.

## 2026-07-06 Bug-log batch â€” GM3 unification, calendar accuracy, buglog summaries, email attribution (4.34.116)

- Scope: all 6 open bug-log tickets (7/6/26), web-side, one pass.
- **NerveCenterNext (`?ui=next`) GM3 pass** (ticket 7KObyXlumpfRTC7ffwHx): per-card accent tints removed â€” `tintedPanel`/`tintedCard` now render ONE neutral surface (page = deeper tonal container, cards = plain surface, GM3 filled-card convention); section accents live only in the icon pucks. Accordion mode fully retired: mobile always gets the card grid, desktop is boxes/full, persisted `accordion` prefs coerce, all accordion selector buttons removed. Header space reclaimed in boxes AND stacked views: the dedicated layout-selector row is merged into the clock row (clock left, one-touch density/layout/actions buttons right) â€” controls stay single-tap, one full row returned to content. Column (3-pane) + tablet views inherit the unified surfaces. NOT done (scope, honestly): restyling the rest of the app beyond NerveCenter â€” that is a separate campaign.
- **Calendar cards** (tickets Q6O8ofo05tskzQtxerpv, 6M5gXtupoKJ5M15O66AC â€” BOTH NerveCenterNext and legacy NerveCenterPanel): events that ended before today are filtered out of `calendarRows` (multi-day/overnight events still running survive; all-day end dates treated as exclusive). Live timeline auto-scroll now parks "now" at the TOP THIRD (was center); agenda lists (boxes card + full view) auto-scroll once per mount so the Now bar also lands in the top third (`agendaNowBarRef` + `data-agenda-scroll`, dataset-guarded so it never fights user scrolling). Full-view agenda separated from the live timeline: its own inset tonal panel (surface-on-surface + radius) with an AGENDA overline, 1px divider removed.
- **Bug Log** (tickets NzMu8aYala8AP7uOGEeY, 2HoPuMCIroUmkN1HwAr2): rows now display a short summary â€” AI-generated via the existing deployed `dashboard.polish_items.v1` job, written once to the bug doc's `summary` field; deterministic first-sentence truncation as instant/fallback display. Full original text on hover (title) and on click (row expands). Expanded rows also show the full work-notes history (`notes[]`) on ANY status incl. resolved â€” the resolution audit trail. Copy-for-team and the reader tool still use ONLY the original `b.text`, never the summary. Reader tool gains `resolve <docId> <note>` (note + resolved in one step) so closing a ticket always records the coder's process.
- **Email summary accuracy** (ticket H9uHoidO4XKFNFHGC2u4): root cause â€” `dashboard.email_summaries.v1` demanded "the sender's voice" while receiving only subject+snippet (no From), so the model guessed authorship. App.jsx now sends From/Date headers; `normalizeEmailInputs` passes them through; prompt rewritten accuracy-first (only stated facts, attribution ONLY from the From field, plain-subject fallback when the snippet is thin). Same attribution rule added to the dashboard signals/chief prompts. Patched in BOTH `apps/web/functions/_ai-core.cjs` (deployed) and `apps/web/backend/functions/_ai-core.cjs`.
- Incident during session: a PowerShell raw-text write mojibake'd `backend/functions/_ai-core.cjs` (PS 5.1 ANSI misread of UTF-8); caught by diff inspection, reverted via `git checkout --`, redone with encoding-safe edits. Lesson: never patch UTF-8 sources with `Get-Content -Raw`/`WriteAllText` in PS 5.1.
- Gates: `npm run build` (apps/web) â†’ 0 errors. BUILD-VERIFIED; tickets closed via reader-tool `resolve` with process notes.
...
- Root cause (code-level, `00-auth.jsx`): `_signInWithGoogle` set ONLY `login_hint` = last-used account â€” Google then re-selects that account silently with no UI. When that account is one Firestore denies (this tablet's documented bad-profile history, HANDOFF Â§1), the sequence is: silent sign-in â†’ App loads â†’ Firestore denies â†’ `handleSessionLostAccess` auto-signs-out with an error â†’ next tap silently picks the same broken account again. An invisible loop with no way to choose a different account â€” exactly the reported symptom. Compounding it, mobile used `signInWithRedirect`, the fragile flow on modern mobile browsers (sessionStorage loss across the redirect â†’ `auth/missing-initial-state`).
- Fix (4.34.115): (1) `prompt: "select_account"` on the login button â€” Google's account picker ALWAYS appears, with the last account pre-highlighted via `login_hint`; the picker is the loop-breaker. (2) Popup-first on every device; automatic `signInWithRedirect` fallback only on `auth/popup-blocked` / `operation-not-supported` / `web-storage-unsupported`. Desktop behavior unchanged (was already popup). (3) Friendly message for `auth/missing-initial-state`.
- Gates: `npm run build` (apps/web) â†’ 0 errors. NOT runtime-verified on the tablet â€” needs one on-device test (expect: picker appears; choosing the correct account signs in). On the branch `claude/phonehost-bluetooth-local-aby9rp`, not pushed to main per this session's branch mandate.
- Dongle spec: storage rule finalized per owner â€” connection data (Wi-Fi, BT bond, API token, sync bookkeeping) MAY persist across power loss; message/call/contact content stays RAM-only; the sole hard constraint is the dongle NEVER talks to Firebase (consuming devices keep their existing Firebase paths unchanged).
- Also reported, NOT in this repo: "rabbimetrics" Google sign-in fails on mobile with a blocked/misdirect error for every account â€” that error text matches OAuth `redirect_uri_mismatch`/origin-not-authorized, a Google Cloud OAuth-client configuration issue (the URL mobile lands on isn't in the client's Authorized JavaScript origins / redirect URIs), not an account or code problem. Candidate GCP projects from the 2026-07-03 audit: `rabbis-metrics` / `rabbi-s-metrics`. Fix lives in that project's Cloud Console â†’ APIs & Services â†’ Credentials â†’ OAuth client â†’ add the exact mobile URL.

## 2026-07-05 (2) Phone-host refinement pass â€” reliability fixes + consumer-grade setup screens
...
- Android host reliability fixes (`apps/phone-host-android`): `map.onStatus` was never wired â€” `/status` reported MAP "Idle" forever; MNS server was created only at service start so a null adapter (Bluetooth off at boot) killed live push permanently â€” now created lazily at connect; `startForeground` guarded (connectedDevice FGS without BLUETOOTH_CONNECT crashes on Android 12+) and BootReceiver/MainActivity no longer start the service unpermissioned; every scheduled task body wrapped in `runCatching` (one throw permanently cancels a ScheduledExecutorService task â€” would have silently killed the watchdog); store saves now atomic (tmp+rename, dirty flag restored on failure) so power loss can't truncate messages.json; sent-echo dedupe in `MessageStore.merge` (send racing the sync produced a duplicate bubble); `HfpClient.connect` closes its socket on handshake failure (leaked RFCOMM channel blocked reconnects); `onDestroy` Bluetooth teardown moved off the main thread (OBEX round-trips under the sync lock â†’ ANR).
- iPad bridge reliability fixes (`apps/ipad-phone-bridge`): proxy work moved off the NWListener serial queue to a concurrent queue â€” the web surface fires /status /messages /calls /contacts in parallel with 2.5 s timeouts and the serial queue would time three out (primary-lane bug); ContentView now observes `probe`/`lanHost` directly (nested ObservableObjects don't propagate through the parent â€” the status card never refreshed).
- Consumer UI rewrite, both platforms: Android `MainActivity` â†’ plain-language status card (Connected to <phone name> / Connectingâ€¦ / Choose your phone / Paused / permission prompt), one contextual action button, device picker by name with âœ“ (no MAC addresses), "Open Bluetooth settings" affordance, all technical detail (addresses, API URLs, protocol log) collapsed behind "Advanced". iPad `ContentView` â†’ "Phone Link" title, single status card ("Connected â€” calls and texts from your phone are available on this iPad"), entire Bluetooth probe/diagnostics moved into a collapsed Advanced DisclosureGroup; app display name "WebPhone Bridge" â†’ "Shamash Phone Link".
- Gates: `gradle :app:assembleDebug` â†’ BUILD SUCCESSFUL after all changes. Swift remains source-only (no Xcode on Linux). Runtime smoke with the real phone still owed (unchanged).

## 2026-07-05 Local BT phone hosts â€” Android host app built; iPad becomes LAN proxy; relay-exit path defined

- Owner request: local Bluetooth phone host for iOS and Android ("starting from iOS then android, seamless switching, then ditch the relay"). Branch `claude/phonehost-bluetooth-local-aby9rp` (not pushed to main â€” native apps, no web release).
- iOS-first is blocked by platform, not by code: MAP/PBAP/HFP are Bluetooth Classic RFCOMM/SDP profiles, NOT GATT services â€” Apple's public CoreBluetooth can never surface them, and iPadOS has no public RFCOMM API (ExternalAccessory = MFi hardware only). The existing `apps/ipad-phone-bridge` probe is exactly this decision gate; verdict recorded in its README. Standalone-iPad would require MFi/hardware-bridge work.
- **NEW `apps/phone-host-android`** â€” the feasible local host, built first: Kotlin app (zero external deps, AGP 8.7.3/Kotlin 2.0.21, minSdk 26) porting the field-proven Windows stack: `ObexClient.kt` (OBEX 1.0 CONNECT, Fig 52/MediaTek header-offset + no-empty-EndBody quirks), `MapClient.kt` (delta sync w/ seeded handles + MaxListCount=1 probes, paginated full-history loader, bMessage/MMS-MIME parser, send w/ SMS_GSMâ†’CDMAâ†’MMS ladder, read/delete pushback, MNS registration), `MnsServer.kt` (RFCOMM 0x1133 OBEX server), `PbapClient.kt` (ich/och/mch call logs + NEW pb.vcf contacts import), `HfpClient.kt` (BRSF=4 â€” deliberately outside the audio path like Windows; RING/+CLIP/+CIEV state machine, ATA/AT+CHUP/ATD control, 30s keepalive probe), JSON-file stores, and `LocalApiServer.kt` on 0.0.0.0:8765 with the Windows ControlApiService contract (same /status /messages /calls /contacts shapes from MainViewModel, same CORS+PNA headers). Foreground service + boot receiver + reconnect watchdog; advertises `_shamash-phonehost._tcp` via NSD.
- **Seamless switching**: one BT client at a time is a profile-level fact, so hosts hand the link over â€” `POST /handoff-release` parks a host (clean HFP/MAP/MNS drop, watchdog off), `POST /connect` on another host takes the link; per-host stores + seeded delta sync make the swap cheap.
- **iPad lane (`apps/ipad-phone-bridge`)**: new `LanHostClient.swift` â€” Bonjour-discovers the active host (manual `POST /lan-host` fallback) and raw-proxies the entire host contract on the iPad's own 127.0.0.1:8765, so the web app works unchanged with NO cloud relay (native URLSession sidesteps the httpsâ†’LAN-IP mixed-content block; loopback is exempt for the web app). Probe endpoints stay local as the gate. pbxproj updated (new file + merged Info.plist with `NSBonjourServices`).
- **Call audio limitation (Android host)**: HFP SCO audio is not app-accessible on Android (no BthHFEnum equivalent) â€” v1 is call control/state only; voice stays on the phone. Documented in the app README.
- Relay exit path: PC and tablet hosts serve their own browser via loopback; iPad consumes whichever host is live via the bridge proxy. Once each device's lane is runtime-verified, `phoneRelay` (Â§15.5) can be decommissioned.
- Gates: `gradle :app:assembleDebug` â†’ BUILD SUCCESSFUL, `app-debug.apk` produced (Linux CI-like env, SDK 34). Swift lane is source-only (no Xcode on Linux â€” same precedent as 2026-05-18). NOT runtime-verified against the real phone yet: first on-device smoke (pairâ†’connectâ†’syncâ†’sendâ†’call state, then handoff between hosts) is the next session's gate.

## 2026-07-03 DeskPhone b326 + RTDB push â€” commands by true stream, zero idle reads (4.34.114)

- Owner GO on replacing the b324 poll with real push. New **Realtime Database** instance `onetaskonly-app-default-rtdb` (us-central1) provisioned via the Management API; rules deployed opening ONLY `phone-relay/commands` (same posture as the old Firestore doc â€” queuing still gated behind Firebase sign-in in the function). `firebase.json` gains the `database` block; rules file `apps/web/database.rules.json`.
- Host b326 (`7b43799`): `StreamRtdbCommandsAsync` holds an SSE stream on the commands node (RTDB REST genuinely streams, unlike Firestore REST listen); put/patch events trigger `DrainMailboxOnceAsync` (GET â†’ PUT null clear-before-dispatch â†’ dispatch; `NormalizeRtdbArray` handles RTDB's object-map arrays). The poll remains as safety net: 120s while the stream is healthy, 2s/6s adaptive when down; plus a 60s legacy sweep of the old Firestore commands doc (remove in a future build). Function (`b268b39`): command/push/drain paths moved to the RTDB node.
- Cost: idle Firestore reads from the command channel â†’ **zero** (was 43k/day originally, 14k/day on b324). RTDB usage rounds to $0.
- VERIFIED: command PUT to the RTDB node â†’ host log `[RELAY CMD] /refresh` in **420 ms**. b326 accepted and running; deploy Action for the function green.
- RETRACTION: the previous entry's "+4h host time skew" claim is doubtful â€” the session spanned many hours and the comparison used a stale local-time assumption; host log clock matches system clock when compared live. Re-verify against a known call before treating call timestamps as a bug.
- Consolidation audit (same session, ydanziger20@gmail.com): 9 GCP projects inventoried; keys/APIs swept. Load-bearing: `onetaskonly-app` (everything), `shamashpro4-health-sync` (project number 684600201717 = the Google Workspace OAuth client), `gen-lang-client-0743523606` (Gemini keys "Onetask App AI Functions", "Message analyzer"), `gen-lang-client-0892770796` (overflow Gemini key). Caution: `primordial-port-459016-s1` and `danziger-kiosk-2026` both have androidmanagement.googleapis.com enabled (possible managed-device policies â€” deleting could brick a kiosk enrollment). Likely junk: `crack-walker-459016-i7` (console cruft only), `rabbis-metrics` (dup of rabbi-s-metrics, telemetry only). NOTHING deleted â€” report delivered to owner; second Google profile not credentialed on this machine, not yet inventoried.

## 2026-07-02 DeskPhone b324/b325 â€” relay command channel was DEAD since b322; restored + hardened (4.33.114)

- ROOT CAUSE FOUND for "no confirmation from DeskPhone", "nervecenter text return not working", fickle web sends, dead reconnect buttons: b322 (6/17) replaced the relay's 2s command poll with a Firestore REST `documents:listen` stream â€” which CANNOT work: the server treats the transcoded bidi call as complete when the request body ends and closes with "[]" (verified via curl: bare-object body â†’ 400 "Invalid value (Object)"; array body + database param â†’ 200 + instant close, zero events). **No relayed command was delivered for 15 days.** First drain on b324 found mailbox commands up to 8.6 DAYS old.
- b324 (`cb86e7d`): RelayService listener â†’ `DrainCommandsLoopAsync` adaptive poll (2s for 3min after a command, 6s idle â‰ˆ 14k reads/day â‰ˆ $0.25/mo Blaze); clear-before-dispatch so slow sends can't re-execute. Relay whitelist gains `/connect` + `/show` (45s TTL for /show), wired to the existing `_api` handlers. Toast quick-replies (ticket: "weren't sending anything") rerouted from raw `_map.SendMessageAsync` â€” which DISCARDED replies whenever MAP was mid-reconnect â€” through the full `SendMessageAsync` pipeline (bubble + Failed/Retry + auto-reconnect). Bundles web dist 4.33.114.
- b325 (`c4c0107`): fail-safe â€” commands with no parseable `queuedAt` are refused as expired (b324's first drain executed unstamped legacy entries; harmless only because they lacked recipients). Live function always stamps queuedAt (`phone-relay.js:153`).
- Web 4.33.114 (`cc6826f`): priority relabel 1/2/3 moved to shared `relabelBuiltinPriorities()` (01-core.js) applied at ALL state entry points â€” the 4.33.113 load-only migration was reverted by every Firestore snapshot (owner: "still labeled now/today"). Open-DeskPhone button un-gated (was `!isMobile` + hidden behind the desktop iframe = invisible everywhere) and routed through `post("/show")` so it works over the relay against b324+.
- VERIFIED END-TO-END: test command written to the `phone-relay/commands` mailbox (proper id+queuedAt) â†’ host log `[RELAY DRAIN] command mailbox reachable` + `[RELAY CMD] /refresh` executed within seconds. Web deploy Action green.
- Gates: `npm run build` (apps/web) â†’ 0 errors; `dotnet build -c Release -p:Platform=ARM64` â†’ 0 errors Ã—2; deploy.ps1 OK Ã—2 (b324, b325 archived; shortcuts + handoff offers issued).
- KNOWN REMAINING (this session's findings): host stamps PBAP call times AND log lines ~+4h in the future (UTC treated as local-04:00 â€” double-stamp bug; makes "Jul 2 11:49 PM" calls at 4:30 PM); NerveCenter visual polish tickets (off-center pill buttons, queue-bar sizing, add-dot styling) and bug-log edit/auto-pull features not yet started.

## 2026-07-02 Bug-log ticket batch â€” phone status/reconnect/texting + tasks (4.33.113)

- Scope: 11 of the 13 open bug-log tickets (7/1/26), all web-side. Commit `45ad492`.
- Phone status: `hostDeviceName` (10-deskphone-web.jsx) now skips raw BT addresses ("7E4B46E95FBA") via `looksLikeBtAddress`; the rail's duplicated two-block status ("Connected to X" + channel row repeating it) merged into ONE plain line; `.dp-channel-*` (which had NO CSS) removed; `.dp-rail-status-text` wraps + left-aligns. Same MAC filter on the NerveCenter phone surface's "Live Â· connected" line.
- Reconnect: the rail sync arrow now POSTs `/connect` (real phone-link reset) instead of a status re-read; the NerveCenter phonescreen header refresh (previously `onClick={refresh}` â€” passed the click event where `forceProbe===true` was required AND never touched the host) is now the ONE reconnect button: `/connect` direct, `/refresh` over the relay (relay whitelist has no `/connect`), then `refresh(true)` re-probe.
...
- Text bubbles: retry affordance on Failed sends and on sends stuck in Sending/Confirming >60s â€” both NerveCenterPhoneSurface (ActionBtn "Retry" resends via `/send`) and DeskPhone web (`.dp-message-retry` in bubble meta â†’ `sendComposeMessage` resend).
- Copy message: `copyTextToClipboard` helper â€” navigator.clipboard with hidden-textarea `execCommand("copy")` fallback (WebView2 permission-blocks the async API).
- NerveCenter reply: expanded-conversation compose box moved BELOW the newest message (was above the whole thread).
- Priorities: built-in labels Now/Today/Eventually â†’ "1"/"2"/"3" (1=highest; ids/weights unchanged). defS + on-load migration in App.jsx (only converts stock labels, preserves custom renames); NC_LABEL pill maps in NerveCenterPanel/NerveCenterNext.
- Stale-task nudge ("waiting N days" popup) removed entirely (state, trigger effect, JSX) per ticket.
- Miscategorization: conversation.extract.v1 gained a shaila-vs-task CLASSIFICATION GUARD â€” patched in BOTH `apps/web/functions/_ai-core.cjs` (Firebase, deployed) and `apps/web/backend/functions/_ai-core.cjs`. Note: NO code path auto-tags Mrs W â€” only the green add-dot next to the priority dots; likely fat-finger, watch recurrence.
...
- Gates: `npm run build` (apps/web) â†’ 0 errors. RUNTIME-VERIFIED in dev preview: suite loads (v4.33.113 in rail), zero console errors; `?standalone=deskphone` renders ONE status line ("Phone service is off â€” open DeskPhone on this PC"), reconnect button present, old channel row gone. Deploy Action `28610352535` green; production bundle `index-CbGuQ332.js` contains 4.33.113.

## 2026-06-21 M3 Phase A (slice 1) â€” buttons: text/action/toolbar + small-file icons â†’ @material/web (4.26.97)

- Scope: M3 component migration **Phase A, slice 1**. Replaced hand-coded button lookalikes with real `@material/web` components: every `gvTextButton` (8 sites: Answer/Decline/Hang up, Cancel, Done, Delete, "More history", row got-back), `cleanToolbarButton` (3 sites: Add, Open Ã—2), and the ConvCapture close (1) + SuitePanels (3 icon + 1 "Position" action) buttons.
...
- Gates: `npm run build` (apps/web) â†’ 0 errors. RUNTIME-VERIFIED in dev preview (port 5178): no console errors; token probe â€” all M3 roles resolve to real theme colors (primary/secondary/tertiary distinct, none transparent); converted "More history" renders as `md-outlined-button` w/ Segoe UI font, theme on-surface color, **exact 31.99px height** (density preserved); record modal close renders as a themed `md-icon-button`. Preview viewport locked at 342px (mobile layout) â€” desktop full-panel buttons not eyeballed here; pending owner desktop QA + the full 8-theme matrix.
- Next: icon buttons (`gvIconButton` Ã—46 + `ncSmallIconBtnStyle`/`phoneIconButton`) in NerveCenterPanel + NerveCenterPhoneSurface â€” Phase A slice 2.

## 2026-06-21 M3 secondary + tertiary accents â€” derived, distinct (4.26.96)

- Scope: secondary/tertiary M3 roles previously mirrored primary (the 8 themes define a single accent). Now derived properly: secondary = same hue at ~45% saturation (muted companion), tertiary = hue +60Â° at capped saturation (distinct complement) â€” Material 3's own model â€” computed from each theme's one primary.
...
- Gates: `npm run build` (apps/web) â†’ 0 errors; `npm run build` (apps/shailos) â†’ 0 errors. RUNTIME-VERIFIED in dev preview: Claude Cream resolves primary `#9A452B` â†’ secondary `#7B554A` (muted brown) â†’ tertiary `#7C9233` (olive), all valid, no console errors, app renders intact. Derivation eyeballed across all 8 theme primaries â€” every theme yields a distinct, harmonious 3-accent set (e.g. googleVoice tealâ†’deep-teal+indigo, material blueâ†’muted-blue+purple, navyGold goldâ†’tan+green); no clashes.
- Note: the full `@material/web` component migration remains deferred â€” see `M3_INTEGRATION_HANDOFF.md` (unchanged).

## 2026-06-21 App-wide Material 3 token bridge â€” theme-reactive, all 3 surfaces (4.26.95)

- Scope: the `--md-sys-color-*` bridge was static and theme-blind â€” it pointed at `--shp-color-*` hardcoded to the googleVoice defaults, with no runtime writer connecting the active theme. Only ~8 of M3's ~40 color roles existed; the rest fell back to M3 default purple. Shailos and DeskPhone web carried no M3 token vocabulary at all.
...
- Web suite (`App.jsx`): render `<style>{themeVarsCss(T)}</style>` immediately after `NC_GLOBAL_CSS` so it wins the cascade â€” the theme-switch hook the global-color-token redesign required. Every `@material/web` component now follows the active theme.
...
- Shailos (`apps/shailos/src/index.css`): added the full M3 `:root` block mapped onto its reactive `--ot-*` layer + `--ot-radius-*` shape scale. Additive only â€” nothing consumes the tokens yet, so zero visual change to Shailos; it now speaks the same M3 vocabulary.
- Scope boundary: this is the token FOUNDATION. It does not convert the ~932 hand-coded `C.*` color usages to `@material/web` components â€” that is the separate component-migration campaign documented in `M3_INTEGRATION_HANDOFF.md`.
- Gates: `npm run build` (apps/web) â†’ 0 errors; `npm run build` (apps/shailos) â†’ 0 errors. RUNTIME-VERIFIED in dev preview (port 5178): probed all derived roles via `background-color` â€” every role resolves to a valid color (no transparent/invalid), no console errors. Theme reactivity proven by switching Claude Cream â†” Navy Gold and reloading: `--md-sys-color-primary` flipped `#9A452B` â†” `#F2C14E`, `surface` `#FFFFFF` â†” `#122235`, and the `color-mix` surface ramp auto-inverted (containers darker in light, lighter in dark). DeskPhone standalone surface: M3 `primary` === `--dp-blue`, all roles valid, shape `medium` = 12px. Original theme restored after testing.

## 2026-06-14 Recording Duplication Fix â€” Prevent existing tasks/shailos from showing in ConvCapture (4.17.66)
...
## 2026-06-14 DeskPhone b321 â€” standalone WebView2 theme propagation & cache-busting (4.17.65)
...
- Gates: `npm run build` â†’ 0 errors, bundle `index-DzbVC8Eo.js`; `dotnet build -c Release` â†’ 0 errors + deploy.ps1 OK. BUILD-VERIFIED â€” b321 deployed, launched, and checked.


## 2026-06-14 DeskPhone b314 â€” message-list responsive layout (4.17.62)

- Scope: finish rate-limit-frozen Cursor session â€” CSS-only fix for the DeskPhone WebView2 message-list column overflow + stacked layout chrome.
- Web (`10-deskphone-web.jsx`, v4.17.62): `dp-message-shell` made a container-query root (`container-type:size`, `container-name:message-shell`). List column changed from `minmax(210px, saved-width)` to `minmax(0, min(saved-width, 36cqw))` so the saved drag width is a cap, not a floor â€” list shrinks proportionally. `@container message-shell (â‰¤720px)` stacks list above thread based on actual shell width instead of viewport; `@container (â‰¤900px)` trims filter buttons and avatars for denser rows. Stacked/narrow states hide filter tabs, history badge, phone-number subtitle; `dp-thread-calls` compacted to `min(152px,28vh)` with muted sidebar bg and small uppercase `CALLS` label; vertical split row heights reduced `220â€“320px â†’ 128â€“240px`.
- Native host (b314): `changelog.json` + `build.num` 314â†’315; Release build archived to `deployed-builds/b314`; desktop shortcuts updated (Latest + Previous b313); deploy commit `009e6ff`. Running b313 received build-update handoff offer.
- Web commit `235986a` pushed to `origin/main`; Netlify Git-triggered deploy initiated.
- Gates: `npm run build` (apps/web) â†’ 0 errors, bundle `index-CjZmGE6O.js`; `dotnet build -c Release` â†’ 0 errors + deploy.ps1 OK. BUILD-VERIFIED ONLY â€” runtime smoke on live DeskPhone WebView2 shell recommended after accepting b314 handoff.

## 2026-06-14 DeskPhone WebView2 â€” WPF UI toggle, resize reflow, thread search scope (4.17.61)

- Scope: finish interrupted session â€” three DeskPhone WebView2 fixes shipped together (web + native host source).
- Web (`10-deskphone-web.jsx`): thread header (including conversation search) moved inside `dp-thread-messages` so search no longer spans the call-history rail; container queries on thread pane/messages stack call history below messages when pane â‰¤700px and wrap header controls when messages column â‰¤520px; flexible grid min-widths replace rigid 340px/260px floors that caused overflow on drag-resize.
- Native host (`ControlApiService.cs`, `MainWindow.xaml.cs`, `MainViewModel.cs`, `WebShellWindow.xaml`): `POST /toggle-main-window` + `mainWindowXamlVisible` in `/status`; Settings â†’ Appearance button shows/hides legacy WPF MainWindow (default hidden); WebView2 stretches/clips to window bounds.
- Gates: `npm run build` (apps/web) â†’ 0 errors; `dotnet build -c Debug` (phone-host-windows) â†’ 0 errors. BUILD-VERIFIED ONLY for resize reflow and WPF toggle â€” runtime smoke on live DeskPhone WebView2 shell recommended.

## 2026-06-14 DeskPhone b313 â€” native Release deploy (WebView2 reflow + WPF toggle)

- Scope: changelog + Release deploy for the b313 host build carrying the WebView2 fixes from 864d953.
- b313 (`changelog.json`, `build.num` 313â†’314): Release build archived to `deployed-builds/b313`; desktop shortcuts updated (Latest + Previous b312); deploy commit `f3d6d37`.
- Gates: `npm run build` â†’ 0 errors; `dotnet build -c Release` â†’ 0 errors + deploy.ps1 OK. RUNTIME-VERIFIED: `http://127.0.0.1:8765/status` reports `"build":"b313  06/14/26 1:19 am"`, `mainWindowXamlVisible:false`. Running instance received build-update handoff offer (user accept required).

## 2026-06-12 One design system â€” DeskPhone teal restyle (b303), webapp embeds DeskPhone (4.17.49), tokens consolidated (4.17.50)

- Scope: three sequential releases. (1) b303: WPF DeskPhone restyled to match the web phone screen exactly â€” pure style, zero functional changes. (2) web 4.17.49: the desktop Phone screen auto-embeds the UI DeskPhone itself serves when the loopback host answers. (3) web 4.17.50: 10-deskphone-web's COLORS palette now derives from ui-tokens GV_CLEAN.
- b303 (`Themes/Palettes/Google.xaml`, `App.xaml`, `MainViewModel.cs`): every blue accent key â†’ teal family (#00796B/#00695C/#E0F2F1/#003731 = GV_CLEAN.accent family); beyond the spec table, the remaining blue-tinted neutrals were also de-blued to the web phone's actual values (sidebar/hover/input surfaces, green/red accents, text greys, BubbleIn/Border) so the app truly matches â€” amber family kept (no web equivalent), BgSelected kept at the specced #E0F2F1 (note: web uses #F1F3F4 there). App.xaml default palette BlueGold â†’ Google (runtime still applies PreferredPalette: light=Google, dark=BlueGold). Removed the b301 OpenModernUiOnLaunch auto-open block; `OpenModernUiCommand` (manual button) stays. Also committed WebShellWindow.xaml(.cs) â€” shipped in b299 but never tracked; the repo did not compile from a clean clone without them.
- 4.17.49 (`App.jsx`, `NerveCenterPhoneSurface.jsx`, `ui-tokens.jsx`): the spec targeted NerveCenterPhoneSurface, but on desktop the full Phone view renders DeskPhoneWebPanel directly in App.jsx (the non-compact surface only renders on mobile, where loopback is unreachable by definition) â€” so the goal was implemented in App.jsx: a 1.5s `/status` probe (re-probe every 25s) swaps DeskPhoneWebPanel for an iframe of `http://127.0.0.1:8765/?standalone=deskphone` with a 200ms `nc-phone-surface-fade` (DUR.base); falls back transparently when the PC dies. The specced surface-level swap was also added (engages via its existing transport resolution). Host verified to send no X-Frame-Options/CSP â€” no host change needed. Chromium-only by design; browsers that block HTTP-loopback frames also fail the probe.
- 4.17.50 (`10-deskphone-web.jsx`): COLORS keeps its role names but 12 of 20 keys now reference GV_CLEAN (bg/hover/bgSoft/accent/accentDark/success/danger/text/muted/faint/divider); 8 literals stay local with no GV_CLEAN equivalent (light accent tints #E0F2F1/#CEEAD6/#FCE8E6, #137333, textSecond #3C4043, textOnAccent, border-strong #BDC1C6). Zero values changed. Audited clean (nothing replaceable without changing values): NerveCenterPanel (category/priority fallbacks), TaskRiverPanel (river lane palette), SuitePanels (error tints), AppSuiteChrome + 06-shelf (hex-free), 04-components (legacy OneTask warm theme â€” different design language by intent).
- Gates: b303 `dotnet build -c Release -p:Platform=ARM64` â†’ 0 errors, deployed + relaunched, `/status` reports "b303 06/12/26 12:02 am". Web: `npm run build` â†’ 0 errors Ã—2. RUNTIME-VERIFIED in dev preview: `?suite=phone` rendered the DeskPhone iframe with live conversations (probe â†’ direct â†’ embed); `?standalone=deskphone` surface resolves --dp-blue #00796B / --dp-border #DADCE0 / surface #FFFFFF from tokens, "This device: b303". Commits 3aa7c5f, 5398ee8, 556325d, a656d5c.

## 2026-06-10 NC boxes view â€” denser task rows, card expand-to-page, Tasks/Shailos icon swap (4.15.37)

- Reported: mobile unstacked (boxes) view tasks list still too spacious â€” halve it (~2 more tasks visible); each card should expand on tap to full page height; Shailos' checklist icon ("rule") should move to Tasks, with Shailos getting a classy question mark.
- Density root cause (shipped from prior session, was uncommitted): `index.html` sets `button{min-height:36px}` globally; min-height beats height, so every icon button shorter than 36px silently propped row heights open. `gvIconButton` now sets inline `minHeight` from its height override (`ui-tokens.jsx`). On top of that, boxes-view task-row Done/Delete buttons shrank 22â†’16px (compact) / 30â†’24px (comfortable), comfortable `rowMinH` 28â†’22.
- Card expand (`NerveCenterPanel.jsx`): new ephemeral `expandedBoxId` state; rows-orientation boxes grid switches to `min-content`/`minmax(0,1fr)` rows. `MobileBox` gained `expanded`/`collapsed`/`onToggleExpand` â€” header tap toggles expand (was: open full surface; that moved to a trailing `open_in_new` button), collapsed cards render header-only via `display:none` content (Phone pollers stay mounted), scroll-away header suspended while expanded/collapsed. Desktop 5-column boxes unchanged.
- Icon swap: Tasks `task_alt`â†’`rule`, Shailos `rule`â†’`question_mark` across NerveCenterPanel (boxes, accordion, full panel, tab bar, action categories, signal chips), AppSuiteChrome rail, SuitePanels Shailos header, App.jsx shaila actions. The "Done" action keeps `task_alt`.
- Gates: `npm run build` (apps/web) â†’ 0 errors. Preview-verified at 420px (desktop boxes layout): expand â†’ 629px card + 22px strips, collapse â†’ even 143px rows; icon names confirmed in a11y tree; 24px icon button computes min-height 24px (was 36).

## 2026-06-09 Mobile NerveCenter â€” Email/Calendar "loading forever" + cards stuck "Waiting on summary"

- Reported: On mobile browser, the Email and Calendar cards spin "Loadingâ€¦" indefinitely, and every card stays on "Waiting on summary" with no summary ever arriving.
- Root cause (shared): no client-side fetch in the Google/AI load path had a timeout. A bare `fetch()` never rejects when a mobile connection stalls mid-flight (radio sleep), so a single stalled request hung forever. Two specific manifestations: (a) the browser-token loader awaited the AI email-summary job *inside* `fetchGmailData`, and set Calendar + Mail state together only after `Promise.allSettled([cal, mail])` settled â€” so a hung AI gateway froze the Calendar card too; (b) `callAIProxy` (NerveCenter `dashboard.nervecenter_summary.v1`) had no timeout, so a stalled AI request left `ncSummaryLoading` true forever â†’ every card's signal note stuck on "Waiting on summary".
- Fix 1 (`01-core.js`): `callAIProxy` now uses an `AbortController` with a 30s timeout (server budget â‰¤26s). A stalled gateway aborts â†’ returns null â†’ callers degrade to "no summary" instead of an infinite spinner.
...
- Fix 4 (`NerveCenterPanel.jsx`): the "Waiting on summary" caption now shows only while `ncSummaryLoading` is true. Once the AI job finishes (even producing nothing / gateway down), the caption clears so cards render their own content instead of a permanently stuck "Waitingâ€¦".
- Gates: `npm run build` (apps/web) â†’ 0 errors, bundle `index-Cke9jSvq.js`. BUILD-VERIFIED ONLY â€” the timeouts/decoupling target a mobile network-stall failure mode that requires runtime verification on the affected device (use `?diag=1`).

## 2026-06-07 DeskPhone Connection Reliability â€” Faster Reconnect + Ghost-Connection Detection (b291)
...
- Root cause (3): Threads and recentCalls both sliced to 10; activity feed capped at 8/14. Fixed: 10â†’20 for threads/calls, 8/14â†’12/20 for activity feed.
- Root cause (4): `MobileSection` header title/preview at 11px, badge at 9px. Fixed: title/previewâ†’13px, badgeâ†’11px. Chief brief signal area label NC_TYPE.smallâ†’meta (13px), note text NC_TYPE.controlâ†’15px. Kept remote's chip-icon design.
- Root cause (5): AI schema described `brief` as "3-5 sentence natural language synthesis" which models render as advisory prose. Fixed: schema and prompt instruction now specify "2-4 short declarative status statements â€” same clipped factual tone as signals, no framing or advice." Remote's Apple-style note description for signals preserved.
- Root cause (7): `_listenV5` in 01-core.js had a `wasInitialized` guard preventing the first Firestore snapshot from calling `onUpdate`. Tasks added on device A between `load()` completing and the first `onSnapshot` firing on device B would land in `taskCache` but never reach the UI â€” invisible until another change triggered a subsequent snapshot. Fix: removed the guard; first snapshot now always calls `onUpdate` when `changed=true`. App.jsx's `adoptedRemote` guard prevents the update from bouncing back as a save.
- Gates: `npm run build` (apps/web) â†’ 0 errors, bundle `index-fFUoLPAJ.js`. Pushed to `origin/main` (commit `1ecba98`). BUILD-VERIFIED ONLY â€” cross-device task sync requires runtime verification with two live devices.

## 2026-06-05 DeskPhone Connection Reliability â€” Auto-Reconnect + Ghost-Connected Fix (b285)
...
- Fix 1 (HFP keepalive â€” `HfpService.cs`): `ReadLoopAsync` now sets a 30s read deadline per iteration via a linked `CancellationTokenSource`. On timeout (no AT traffic for 30s, not user-cancel), sends `AT+NREC=0` as a liveness probe. If the write throws (socket dead), exception propagates to outer catch â†’ `StatusChanged("HFP disconnected")` fires. Ghost-connection detection window: max 30s (was: never / up to 120s).
- Fix 2 (HFP disconnect triggers reconnect â€” `MainViewModel.cs` `StatusHfp` setter): Added `QueueAutoReconnect("HFP status: " + value)` call when the new status is non-connected. Existing guards in `QueueAutoReconnect` (`IsConnecting`, `_nextAutoReconnectUtc`, `_autoReconnectInFlight`) make this safe: fires are suppressed during active connect/reset, during the 90s rate-limit window, and if a reconnect is already in flight.
- Fix 3 (persistent watchdog â€” `MainViewModel.cs`): Added `_appCts` (app-lifetime `CancellationTokenSource`, cancelled only in `Shutdown()`) and `StartConnectionWatchdog`. Watchdog runs every 30s on a background thread, calls `QueueAutoReconnect` when `!IsFullyConnected && !IsConnecting`. Because it uses `_appCts` (not `_sessionCts`), it survives any number of failed reconnect attempts that cancel `_sessionCts`. Started from `InitializeDataAsync` after initial auto-connect. Cancelled and disposed in `Shutdown()`.
- Gates: `dotnet build DeskPhone.csproj -p:Platform=ARM64 -c Release` â†’ 0 errors, 2 pre-existing NU1603 warnings. b285 deployed and running DeskPhone notified. BUILD-VERIFIED ONLY â€” end-to-end (real drop + recovery) requires runtime verification on device.
...
- Root cause (2): the relay `state` doc carried no freshness marker. The webapp lit the green "Live" cloud the instant it read the doc, even if the PC closed hours earlier â€” no way to distinguish a live link from a stale snapshot.
- Fix (1) â€” auth/profile (user chose "Auto + remember Google"): `00-auth.jsx` remembers the last working Google email (`ot_last_google_email`) on popup + redirect sign-in and pre-selects it via Google `login_hint`; AuthGate gains `handleSessionLostAccess` â†’ sets a recovery notice + `signOut()` so the user lands on Google sign-in (one tap). `App.jsx` load effect now classifies a no-data load: it runs `Store.probeFirestore()` (authoritative REST probe) and only on a DEFINITIVE 401/403 DENIED â€” never a network BLOCK, never when usable local data exists, never for the dev user â€” calls `onSessionLostAccess()`. Guarded by sessionStorage `ot_access_recovery` (set on recovery, cleared on a confirmed-good load) so it can't loop. Added a `.catch` so a rejected load (the common permission-denied case) also recovers/falls back instead of hanging.
- Fix (2) â€” phone-relay liveness: `phone-relay.mjs` `push` now stamps `relayReceivedAt = Date.now()` into the state blob server-side (works without a DeskPhone rebuild; independent of the PC clock; falls back to raw store if the body isn't JSON). `NerveCenterPhoneSurface.jsx` reads `relayReceivedAt` from both the REST poll and the onSnapshot listener, ticks a 5s clock, and derives `relayStale`/`phoneLinkLive` against a 30s window (DeskPhone heartbeats ~5s). The control-bar indicator now shows green "Live" vs amber "cloud_off Â· {age}", a prominent stale banner appears when the PC looks offline, and `onOnlineChange`/`onStatusSummary`/`phoneActivitySnapshot` report true liveness so the NerveCenter tile can't claim "connected" over data from a closed PC. LAN/desktop path unchanged (liveness = the just-completed `/status` fetch).
- Gates: `npm run build` (apps/web) â†’ 0 errors, bundle `index-CKuGaXfB.js`; markers `relayReceivedAt`/`cloud_off`/`PC offline`/`ot_access_recovery`/`login_hint`/`probeFirestore` present. `node --check phone-relay.mjs` OK. BUILD-VERIFIED ONLY â€” not yet runtime-verified on a real device / live Netlify deploy. The relay stamp ships in the Netlify function (deploys with the web build); no DeskPhone rebuild required for liveness.

## 2026-06-04 Phone Relay â€” Real-Time Push (kill the 6.5s poll, push on change)

- Reported: remote phone-bridge relay (phone â†’ PC â†’ cloud â†’ all signed-in devices) works glitchily/incompletely. Root cause was architectural, not a bug: both ends were timer-based. DeskPhone pushed state every 5s regardless of changes; every browser polled `phone-relay?action=state` every 6.5s. New-text worst case â‰ˆ11.5s, and a dropped client poll/listener went silently stale (same terminal-listener class as the shailos/V5 fixes).
- DeskPhone (`Services/RelayService.cs`): added `PushNow()` â€” a coalescing throttle (idle â†’ immediate; mid-burst â†’ single trailing push â‰¥900ms apart, blob rebuilt from live callbacks at send time so the latest state always lands). All pushes (5s heartbeat + PushNow) serialize through a `SemaphoreSlim` gate so two writes never race the doc. Relayed commands now `PushNow()` their result so remote call/text actions reflect in ~1s. Cloud blob trimmed 500 â†’ 150 messages (onSnapshot re-sends the whole doc to every device per change â†’ keep it small/mobile-data-cheap, well under Firestore's 1 MiB doc cap; LAN `/messages` path still serves full 5000).
- DeskPhone (`ViewModels/MainViewModel.cs`): `RebuildConversations()` â€” the single funnel for every inbound/sent/read-state change â€” now calls `_relay.PushNow()`.
- Web (`08-app-split/components/NerveCenterPhoneSurface.jsx`): in relay mode, replaced reliance on the 6.5s poll with a real-time Firestore `onSnapshot` listener on `phone-relay/state`. Mirrors `Store.listenShailos`: self-resubscribes on capped exponential backoff because an onSnapshot listener is TERMINAL after its error callback. Extracted shared `applyPhoneState()` so poll and listener apply data identically; existing signature refs dedupe redundant renders, so both sources coexist safely (poll stays as a safety net). Net new-text latency â‰ˆ1â€“2s to every signed-in device; dropped listeners self-heal.
- Security: read path unchanged â€” `onSnapshot` reads through the existing deployed rule `match /phone-relay/state { allow read: if request.auth != null }` (same rule the REST relay read used), present in `apps/shailos/firestore.rules`. No rules change needed. Pre-existing posture noted (not introduced here): `phone-relay/state` write is open at the Firestore layer (gated only by the function's `X-Relay-Secret`); textbook hardening would move the function to the Firebase Admin SDK so the doc can be `allow write: if false`. Deferred.
- Gates: `npm run build` (apps/web) â†’ 0 errors, bundle `index-e5dFDoZ6.js`; `relay listener error` + `phone-relay` markers present in shipped JS. `dotnet build DeskPhone.csproj` â†’ 0 errors, 15 pre-existing warnings. BUILD-VERIFIED ONLY â€” end-to-end (real phone + running DeskPhone + deployed Netlify) not yet runtime-verified on-device. The PushNow half ships in the DeskPhone `.exe`, which must be rebuilt/redeployed separately; a web-only deploy makes browsers real-time but the PC still pushes on the 5s heartbeat until the host is updated.
...
- New `?diag=1` overlay (src/diagnostics.jsx, mounted from AuthGate) reports build commit/time, account uid, online state, latest-snapshot source (server vs CACHE), last server-sync age, load status, and whether a SW controls the page â€” plus Force-resync, Reset-Firestore-cache, and full-reset (unregister SW + clear caches + reload) buttons. This makes staleness observable from the device instead of guessed remotely.
...
## 2026-06-01 Round 2 â€” Live-Listener Resilience, Backgrounded-PWA Staleness, Review Triage
...
- Found the real cause of persisting staleness: the task live-listener `_listenV5` (01-core.js) had an EMPTY error handler `}, () => {}` for BOTH the tasks and settings `onSnapshot` listeners â€” same terminal-listener bug as shailos, but only shailos was fixed in round 1. A transport drop (backgrounded mobile PWA, WebChannel kill) killed the task stream permanently â†’ frozen on cached data. Fixed: both now resubscribe with capped exponential backoff; cleanup stops retries.
- Added a foreground/online Firestore reconnect (01-core.js init): backgrounded mobile PWAs get a silently-stale stream on resume; `disableNetwork()â†’enableNetwork()` on `visibilitychangeâ†’visible` (after >30s hidden) and on `online` forces a fresh connection and flushes queued writes.
- Closed a data-loss path amplified by round-1's resubscribe: the shailaâ†’task sync (App.jsx ~1007) deleted tasks whose `shailaId` was absent from a snapshot; resubscribe re-emits CACHED snapshots, so a stale/partial cache could wrongly delete shaila tasks. Threaded `snap.metadata.fromCache` through `listenShailos` and now skip deletions on cached snapshots (server-confirmed only). This also fixes a pre-existing latent bug.
- `postJson` (10-deskphone-web.jsx) had no timeout/abort (unlike `readJson`) â€” a hung DeskPhone froze the UI on POST commands. Added the same AbortController timeout (HOST_FETCH_TIMEOUT_MS).
- Service worker `networkFirst` (public/sw.js) had no fetch timeout â€” slow/half-open connections hung navigation. Bounded to 6s, then cache fallback.
...
- Reported regressions: Firebase not connecting / shailos empty (worked the prior day), Android installed app serving very stale data, mobile login broken, mobile Tasks Â·Â·Â· menu not working, and the NerveCenter Chief tile saying "quiet" when the phone is simply not connected.
...
- Root-cause map: (a) iOS/Android WebChannel transport gets killed â†’ "can't reach Firebase" + Firestore serves stale IndexedDB cache; (b) `listenShailos` onSnapshot is terminal on error and never resubscribed, and shailos have no localStorage fallback, so one transport drop empties the lane permanently; (c) service worker `cacheFirst` assets never purged (static `CACHE_NAME`); (d) mobile `MobileSection` was defined inside render â†’ recreated every clock tick â†’ remount dropped taps/keystrokes (broken Â·Â·Â· menu + task composer focus loss); (e) mobile `signInWithRedirect` errors were only `console.warn`-ed, leaving the user on a silent login screen; (f) Chief snapshot tile showed "quiet" regardless of phone connectivity.
- Fixes (apps/web): `01-core.js` re-added `db.settings({ experimentalAutoDetectLongPolling: true, merge: true })` (desktop keeps WebChannel, blocked networks fall back to long-polling) and rewrote `listenShailos` to self-resubscribe with capped exponential backoff so a transport hiccup can't empty shailos; `public/sw.js` bumped `CACHE_NAME` to `onetask-offline-v2` so installed PWAs purge stale bundles on activate; `NerveCenterPanel.jsx` hoisted `MobileSection` to module scope (state via props) so it re-renders instead of remounting, and made the Phone Chief tile read `phoneActivitySummary.online` â†’ "not connected" vs "quiet"; `00-auth.jsx` surfaces redirect sign-in errors (`unauthorized-domain`, `web-storage-unsupported`, generic) to the LoginScreen instead of swallowing them.
...
- `git diff --check` clean. Pushed to branch `claude/firebase-shailos-debug-Zm62h` (NOT main â€” needs merge to main for Netlify to deploy).
- Action item NOT fixable in code: the default `*.firebaseapp.com` authDomain + Safari/iOS tracking-prevention is the likely deeper mobile-login blocker; verify `onetaskfocuser.netlify.app` and `onetaskonly-app.firebaseapp.com` are in Firebase â†’ Auth â†’ Settings â†’ Authorized domains. On-device verification of mobile menu/login still required (cannot reproduce iOS/Android from CI container).
...
- Updated `NerveCenterPhoneSurface.jsx` to extract `lanUrl` from the relay state blob and surface a `Use direct` button in the relay panel â€” one click saves the LAN URL as the remote DeskPhone URL, dropping the relay entirely for zero-latency LAN access.
...
- Fixed `NerveCenterPhoneSurface.jsx` relay error mapping to distinguish `relay:denied` (Firestore rules) from `relay:no_auth` (token missing/invalid) and the generic `relay:fail` bucket â€” user now sees "Relay blocked by Firestore rules â€” set allow read, write: if true for phone-relay collection." instead of "Cloud relay unreachable."
...
- Firestore security rules required for relay: `match /phone-relay/{doc} { allow read: if request.auth != null; allow write: if true; }` â€” commands doc also needs `allow read: if true` for DeskPhone drain.
...
- Pushed commit `d8cbbb1` to `origin/main`; Netlify Git-triggered production served `assets/index-CK2_Vva9.js` (HTTP 200) â€” hash matches local build exactly.
...
- Host b300: `Open Shamash Web UI` button and POST `/open-web-ui` now open the production app (`https://onetaskfocuser.netlify.app/?suite=phone`) in the DEFAULT BROWSER â€” the user's signed-in session lives there, fixing the logged-out embedded copy from b299. The WebView2 shell remains only for the Call Audio Console (loopback, no auth). Release ARM64 build passed; deploy archived `deployed-builds/b300`, launched it (`build: b300`, `hfp: HFP connected`); release commit `98880e8` pushed.
...
- Live diagnosis: DeskPhone PID alive (started 17:19) but NOTHING listening on 127.0.0.1:8765 â€” netstat showed only SYN_SENT clients; every /status attempt refused. Webapp transport resolver therefore correctly fell back to the relay, whose state was stale â†’ "Waiting for your PC".
- Root cause: `MainWindow.OnClosed` â†’ `vm.Shutdown()` â†’ `_api.Stop()` kills the control listener. Pre-b301 the process then exited (WPF default ShutdownMode=OnLastWindowClose; MainWindow was the only window). b301's auto-opened `WebShellWindow` kept the process alive past Shutdown â€” closing the classic window now produced a zombie: engine dead, windows open.
- Fix (b302): `App.xaml` `ShutdownMode="OnMainWindowClose"` â€” closing the classic main window exits the whole app (shell windows included), restoring pre-b301 semantics; closing only the modern window touches nothing.
- Immediate remediation during the incident: restarted DeskPhone (b301) â€” listener and webapp recovered within one poll cycle. Then released b302; verified `build: b302`, `hfp: HFP connected`, port 8765 LISTENING, modern UI auto-open intact. Release commit pushed (`3c0faef`).
- Also captured in the zombie's log: `[RELAY MEDIA] HTTP 401` hot retry loop (~5.5 s) for three media ids â€” pre-existing relay-media auth bug, flagged as a separate task.

## 2026-07-01 From-scratch M3 NerveCenter behind ?ui=next (web 4.32.106)

- Restored the project's lost local git repo: no `.git` existed in `Shamash Pro 4 App` (commands resolved to a stray empty repo in the home dir), so the tree was disconnected from `origin/main`. Re-`init`ed, re-added `origin`, fetched, and `reset --mixed origin/main` to reattach WITHOUT touching the working tree. Baseline back at `56140b9`; rollback restored.
- Committed the pre-existing uncommitted Bug Log feature as its own checkpoint (`f88ec4b`, feat) â€” floating Firestore-backed bug/idea widget (`users/{uid}/bugs`), Store.addBug/updateBug/deleteBug + BugLog.jsx wired into App on every screen.
- Began the owner's "tear down and rebuild the whole UI from scratch in Google Material 3 pro-level" project. Approach (owner-approved): new UI built in parallel behind `?ui=next`, swapped surface-by-surface, backend ("pipe & plug") untouched; NerveCenter first.
- New from-scratch M3 (Expressive) surface: `09-next/system/kit.jsx` (Card / IconPuck / CountPill / EmptyState / QuickBar design kit, theme-driven so all 8 themes work) + `09-next/surfaces/NerveCenterNext.jsx` (reimagined dashboard: hero clock + at-a-glance stat pills, Tasks hero card with priority filter chips + inline edit + composer, Shailos, Phone, Calendar/Mail, Command tiles). Extended `m3.jsx` with real `md-outlined-text-field` + `md-switch` wrappers.
- Wiring: NerveCenterNext takes the EXACT same props contract App hands the legacy NerveCenterPanel â€” every button (add/complete/delete/edit task, MrsW, zen/queue/bulk/braindump/shatter, shailos add/followup/record, phone open/record call/conversation, Google connect/refresh/settings, Chief, Health sync, Polish) calls the real existing handler. Pure view-layer swap in `App.jsx`: `NerveSurface = (UI_NEXT && suiteView==="nervecenter") ? NerveCenterNext : NerveCenterPanel`. Legacy is the default; production untouched unless `?ui=next`.
- `npm run build` passed in `apps/web` (238 modules; existing large-bundle warning only). Runtime pixel/8-theme verification pending: NerveCenter sits behind AuthGate, so the owner will visually review the live `?ui=next` (as requested). The surface is flag-gated, so shipping it carries zero risk to the default production UI.
- Version bumped 4.30.106 â†’ 4.32.106 (two feats: BugLog + rebuild). Pushing to `origin/main` also ships the BugLog widget live (ancestor commit).

## 2026-07-06 Bug-Log Triage Pass â€” 22 Tickets Closed Out (web 4.34.115)

- Fixed the bug-log reader tool (`tools/bug-log-reader`): it queried Firestore by the Firebase Auth uid, but the app keys user data by email prefix (canonicalUid) and the owner uses the rabbi account â€” bugs live at `users/rabbidanziger/bugs`. Reader now targets that path and gained `note <id> <text>` and `status <id> <status>` write commands, satisfying the "connect the buglog for autopull" and "edit/update function" tickets.
- Queue list container, empty state, and four insights cards in `App.jsx` used `borderRadius:RADIUS.pill` with `overflow:hidden` â€” the "task queue is a big pill with top/bottom cut off" ticket. All switched to `RADIUS.md`.
- The full-width `md-list` "Queue Â· N" shortcut row on the tasks main page replaced with a centered `md-assist-chip` (the "queue view bar too long, was a pill" ticket). Verified visually in dev.
- NerveCenter task-add controls (both NerveCenterPanel and NerveCenterNext): `compactAddDot` dropped its tinted pill background/border; icon changed from `add` to `check` â€” bare colored checkmarks per the owner's ticket. Verified visually in dev.
- Missed calls: the phone card feed sorted texts+calls by timestamp and capped compact view at 12 items, so missed calls could be crowded out entirely ("MISSED CALLS GONE"). Unresolved missed calls are now pinned to the top of the feed, exempt from the cap. Verified in dev with ?ncmock=1 (missed call w/ "Needs callback" pinned first).
- Verified live against host b326 (`/?standalone=deskphone`): status line reads "Your phone is out of range â€” reconnects when nearby", no MAC addresses anywhere, single reconnect arrow wired to `/connect` â€” the phone-status "gibberish"/reconnect/justification tickets were fixed by 4.33.113/114 + b324â€“b326 and confirmed on-screen.
- `npm run build` passed in `apps/web`; version 4.34.114 â†’ 4.34.115 (fix).

## 2026-07-10 Phone-Connection Ticket Batch â€” Sends, Doubles, Toggle, Status Truth (web 4.36.125)

- Session context: Claude-on-web session (no Firebase credentials, no live devices), working the five open 7/7â€“7/9 phone tickets on branch `claude/shamash-phone-connection-bugs-nc0cz6`. Verification = build + module unit tests + headless-Chromium shell smoke; on-device confirmation still needed (see "pending" below).
- **Instant sends everywhere (ticket: "text freezes 5 secondsâ€¦ do it immediately in all sms sending fields")**: new shared echo store `apps/web/src/08-app-split/utils/pending-sms.js` â€” every composer (NerveCenter phone surface compose/reply, DeskPhone web reply, new-message tab, retry buttons) now drops the typed text into the thread instantly as a "Sendingâ€¦" bubble, clears the box, and runs the command in the background (`post`/`runCommand` gained a background mode that skips the busy-freeze and the global error banner). Failure flips the bubble to "Failed" + Retry, so nothing typed is ever lost; success leaves a plain sent bubble until the host's own copy arrives and the echo auto-drops (match = normalized recipient + whitespace-tolerant body + 10-min skew window).
- **Send-doubling (ticket: "one says sent the other forever on confirming on phone")**, three layers:
  1. Windows host `MessageStoreService.Merge`: the local-bubble â†” phone-sent-copy reconcile window was a flat 90 s of PC-clock-vs-phone-clock delta â€” ordinary clock skew beat it and the "Confirming" bubble never adopted the phone copy. Now 15 min while the local bubble still carries an in-flight SendStatus (only our own recent send can look like that), 90 s otherwise. **Needs a PC host rebuild (next b3xx) to take effect.**
  2. Android host `MessageStore.merge`: echo adoption required EXACT body equality; phone sent-folder copies differ in \r\n/whitespace. Now whitespace/line-ending-tolerant, and a "confirming" status also resolves to "sent" on adoption. **Needs a tablet host rebuild.**
  3. Web (`collapseHostDoubles`, live immediately): both thread builders visually collapse an outgoing bubble stuck on Sending/Confirming when a confirmed copy of the same text exists in the same thread within 10 min â€” so existing stuck doubles in old host blobs disappear from every browser today.
- **Rail host toggle (ticket: "huge and ugly and not gm3â€¦ idk if it even works")**: still the real `md-switch` (M3 rule) but sized to rail density via official md-switch tokens (40Ã—22 track, 30px state layer), `flexShrink:0` so it can't squish, theme-bridged unselected track/outline. Label now shows the host that ACTUALLY holds the phone (owner-doc heartbeat) and reads "Handing to PCâ€¦/Tabletâ€¦" (italic) while the flip hasn't landed â€” the toggle visibly does something the moment it's tapped, and honestly shows when the hosts haven't obeyed yet.
- **Status truth (tickets: "says can't connect but shows recent calls from 4 hours ago", "no status indicator")**: when the link is down but old data is on screen the one status line now says "Offline â€” showing texts & calls from 4h ago"; transient refresh failures with data on screen no longer paint the red "can't reach" banner over a live-looking feed (hard failures and actionable states â€” sign-in, rules, no host â€” still do). Connected line appends "handing to Xâ€¦" during a host handoff.
- Verified: `npm run build` green (2Ã—); `node` unit run of pending-sms (echo add/match/reconcile/expiry, phone/body normalization, double-collapse keeps confirmed + unrelated, drops stuck double) â€” 9/9 ok; built bundle served via `vite preview` + headless Chromium: suite shell, rail (compact switch, "Phone: Tablet" label), and phone card ("Sign in to see your phone here." auth state) all render with zero page JS errors (network errors expected â€” sandbox blocks Firebase/fonts).
- Pending on-device: host rebuilds (PC + tablet) for the root-cause reconcile fixes; live relay send from iPad/desktop to confirm echo â†’ host-copy handoff timing; visual pass on the resized switch in all 8 themes.
- Version 4.36.124 â†’ 4.36.125 (fix).

## 2026-07-10 Phone-Link Overhaul â€” One Truth Machine, Exact Send IDs, Never-Blank (web 4.37.125)

- Follow-through on the second-opinion review of the 4.36.125 ticket batch: replace compensations with architecture. Device roles confirmed by owner (phone = no apps possible â†’ BT host mandatory; Galaxy tablet = primary host; PC = standby + call audio; iPad = web consumer), so the tablet-primary arbitration stays â€” no from-scratch rebuild.
- **Repaint bug (found in review, fixed)**: both surfaces' message-list change detection was `length + first/last id`, so a send-status or read-state flip mid-list NEVER repainted â€” a text could stay visually "Confirming" long after the host confirmed it. New `messageListSignature()` hashes id+sendStatus+read of every entry; wired into `NerveCenterPhoneSurface.applyPhoneState` and the DeskPhone page refresh.
- **One status truth**: new pure module `apps/web/src/08-app-split/phone-link.js` â€” single state machine (`connected / switching / offline / connecting / no-host` + data age) and single wording source (`describePhoneLink`). Both surfaces now derive every dot/label from it; the DeskPhone page's separate 45 s window is gone (it also now subscribes to the owner heartbeat doc), so two screens can no longer disagree about liveness. `phone-host-control.js` re-exports the constants from it (one source).
- **Never-blank rule**: neither surface wipes messages/calls/contacts on fetch failures anymore (NerveCenter used to blank after 3 bad cycles; DeskPhone page on ANY error). Old data stays with "Offline â€” showing texts & calls from Xh ago"; loopback-path liveness now keyed to last-successful-fetch age so a dead local host can't show a stuck green light.
- **Exact send identity (cid)**: every composer's echo id now rides `/send?...&cid=` (and the attachments JSON) â†’ both hosts stamp it as the message's own id (Windows `LocalId`, preserved across phone-copy adoption; Android `localId`) â†’ the state blob returns it â†’ `pending-sms.js` reconciles echoâ†”host-copy by exact id. Fuzzy recipient+body+time matching demoted to fallback for pre-rebuild hosts. Windows delegates changed: `Send(to, body, cid?)`, `SendWithAttachments(to, body, attachments, cid?)` in ControlApiService + RelayService + MainViewModel.
- **Diagnosis + tests + cleanup**: `?phonediag=1` overlay on the phone surface shows the raw machine (state, owner doc, heartbeat ages, echo list) for minutes-not-guesswork triage; committed `npm run test:phone` suite (node:test, 19 tests â€” machine states, wording, signature repaint, cid + fuzzy reconcile, double-collapse) â€” all pass; dead `DeskPhoneMiniDock.jsx` deleted.
- Verified: 19/19 tests green; `npm run build` green; headless-Chromium smoke of the built bundle renders shell/rail/phone card with zero page JS errors (sandbox blocks Firebase/fonts as always).
- Pending on PC (see `docs/ops/PC_HANDOFF_PHONELINK.md` â€” step-by-step): rebuild DeskPhone (b329) + tablet APK to activate cid + reconcile fixes; then a follow-up session deletes the legacy fallbacks listed in that doc Â§5.
- Version 4.36.125 â†’ 4.37.125 (feat).

## 2026-07-12 Pre-Production Deep Pass â€” GM3 Sweep + Ticket Batch + Multi-User Rules (web 4.42.131)

- Session scope (owner: "deep final dive"): remaining GM3 holdouts on production surfaces, open buglog tickets, and multi-user auth compartmentalization â€” one pass, evaluated and executed together.
- **Handoff truth (ticket r0wEKJ7)**: new `handover` state in `phone-link.js` â€” for 2.5 min after the preferred-host flip, a dead link whose owner doc still names the old host reads "Handing to PC â€” waiting for it to confirmâ€¦" instead of "Offline/lost connection"; past the grace window honesty resumes (Offline + Reconnect). `normalizeOwner` now carries `preferredAtMs` (the toggle already wrote it).
- **History wipe on handoff (ticket LDu4QWw)**: new pure `mergeMessageFeeds`/`mergeCallFeeds` in `phone-link.js` â€” both surfaces union the incoming host list with what was already on screen (incoming wins per id; fuzzy body+peer+2-min window dedupes the same SMS under two hosts' id schemes; 2000-entry cap). A freshly handed-to host re-syncing from the phone no longer blanks the feed. Wired into `NerveCenterPhoneSurface.applyPhoneState` and the DeskPhone web page refresh (both transports).
- **Calendar events missing (tickets rRYEUOn + Bm7Phcr)**: the merged multi-calendar list was capped at 20 events sorted by start time â€” a zmanim calendar filled all 20 before evening, silently dropping shkia/mincha/later-day events. Caps raised 20â†’120 (per-calendar maxResults 25/50â†’100) in ALL THREE fetch paths: `App.jsx` client, `functions/google-workspace.js` (live Firebase), `backend/functions/google-workspace.js` (Netlify copy).
- **Full/Focus toggle removed (ticket 5ORgAKX)**: NerveCenterNext is always Full now; the FULL/FOCUS mini-segment is gone.
- **Account picker on every layout (ticket HwhngHW)**: the Google account picker was hoisted to one shared element and now appears in all four NerveCenter layouts â€” desktop card strip (Calendar + Mail), box layout (Calendar toolbar + new Mail toolbar), and the compact list layout (as radio menu-items in Calendar/Mail section menus).
- **Stale active call (ticket 0kti1vt, web half)**: pressing Hang up / Decline now optimistically suppresses the host's lagging "Active" callState (self-lifts on any NEW host state or after 90 s) â€” no more stuck "On call" card after ending a call on the phone. The missed-vs-answered log classification half lives in the host HFP handler â€” next tablet/PC host build.
- **GM3 sweep (production surfaces)**: every remaining hand-rolled icon/text button converted to the real `@material/web` layer (m3.jsx IconBtn/ActionBtn): NerveCenterNext (Chief page header/composers/suggestion rows/profile, "Try again", "Brief me", autorenew buttons), NerveCenterPhoneSurface (all 18 `phoneIconButton`/`gvIconButton` call sites â†’ `PhoneIconBtn` on md-icon-button), SuitePanels (Dock/Open/Release), App.jsx tasks screen (opt/first-step/list-name/restore/delete-list/shaila-delete modal button rows, 3 undo snackbars, queue action rails on rows/groups/subtasks, AI-prioritize + Settings gear, search clear, quick-add submit) and the Shailos screen chrome (Backup/Restore/Sync/Close). Sanctioned fallbacks kept per CLAUDE.md: priority circles/chips, pane resizers, card headers, nav segments, custom action sheet. Known remaining holdouts (documented, next pass): two `<select>`s on the Chief page (â†’ md-outlined-select), legacy `?ui=legacy` surface, 10-deskphone-web page internals.
- **Multi-user auth compartmentalization (rules deployed live)**: `firestore.rules` rewritten â€” any verified Google user now reads/writes ONLY `users/{their-email-prefix}` (matches `canonicalUid`); the owner's two accounts (rabbi + ydanziger20) are admin over everything; `phone-relay/state` + `phone-media` reads tightened to owner-only (the phone-relay function verifies callers by reading state WITH their token, so this one rule also locks the function's state/command endpoints â€” other users' phone card correctly shows no link, and they can no longer read the owner's texts or queue SMS commands). Host-written docs (state/media writes, commands, owner) stay open â€” hosts push with only the public API key; tightening those needs the host-rebuild security followup. Known limitation documented in the rules: same-prefix-different-domain collision; durable fix = uid-keyed storage migration (guarded refactor, deliberately NOT done here). Rules validated (firebase_validate_security_rules) and deployed (job 1783897493640, success). Google Workspace refresh tokens confirmed server-only (`serverOnlyGoogleWorkspaceTokens`, no client rule = default deny).
- Verified: `npm run build` green (multiple gates through the pass); `npm run test:phone` 27/27 (8 new tests: handover grace/expiry/completed-handoff, message retention/fuzzy dedupe/id-wins/empty-body guard, call retention). Runtime visual pass on the converted M3 buttons pending owner's next live session (bridge tokens already load-bearing for these component types).
- Version 4.41.131 â†’ 4.42.131 (feat; sits atop the parallel host-session commits b334/b8 that landed mid-pass).


## 2026-07-13 Shailos Goes Native — Full Integration, Iframe Retired (web 4.43.132)

- Owner directive: "the shailos app is part of this app now — GM3 it purely and integrate the whole thing, drop the iframe method."
- **Native port**: `apps/shailos` (React+TS+Tailwind standalone, served from `/shailos/` in an iframe) is now `apps/web/src/08-app-split/components/ShailosTracker.jsx` + `apps/web/src/08-app-split/shailos-ai.js` — a 1:1 functional port of App.tsx/geminiService.ts: record-shaila mic capture, call recording (delegates to ConvCapture via `onRecordCall`; system-audio mix retained as standalone fallback), IndexedDB transcription holding pen (SAME database name, so held audio survives the migration), AI parse → duplicate-match → integrate/create flow, per-field mic dictation (date/synopsis/asker/answer/content), AI synopsis + answer-summary regeneration, web research pipeline (queries → google-search proxy → follow-ups → summarized sources + Sefaria deep links), status workflow (pending/answered/got_back), search + status filters, copy-all export.
- **Same data, same wire**: reads/writes the identical Firestore docs (`users/{canonicalUid}/shailos`) via the main app's compat SDK + `db` instance (no second Firebase client, no second auth listener — the signed-in `user` comes in as a prop); AI still routes through the auth-gated `/api/ai-proxy` + `/api/google-search` with the shared `onetask_ai_config`.
- **Pure GM3**: Tailwind/motion/date-fns/clsx are gone. All controls are real `@material/web` via m3.jsx — ActionBtn/IconBtn everywhere, `md-filter-chip` status filters, `md-assist-chip` got-back toggles, `md-outlined-text-field` (incl. textarea) for question/answer/answerer/raw/paste/search, `md-circular-progress` spinners. Theme comes from `cleanTheme(T)` tokens directly (the old localStorage `onetask_theme` handshake + --ot-* CSS-var bridge is obsolete). Borderless inline edits (date/synopsis/asker) are token-styled per the fallback rule. TextField wrapper gained `onBlur` (mapped to composed `focusout`) for the save-on-blur contract.
- **Iframe machinery removed**: both mount points (SuiteShailosPanel and the App.jsx Shaila Transcriber overlay) render the component; the `postMessage` close/open-conv-capture handshake and `?action=` URL param became plain props; `copy-shailos-to-dist` dropped from `.github/workflows/deploy.yml`; `/shailos/**` rewrite removed from firebase.json (old bookmarks fall through to the app shell). `apps/shailos` stays in the repo as the historical source; it just no longer ships.
- Verified: `npm run build` green in `apps/web`; `npm run test:phone` 27/27. Pending owner smoke on live: open Shailos from the rail (list loads, edit/save round-trips), one dictation, one research run.
- Version 4.42.132 → 4.43.132 (feat).

## 2026-07-13 Three-Lane Phone Link — iPad Cloud Lane, Auto-Finder, Live Call Feed Record (web 4.44.132)

- Session scope (owner directive, one pass): iPad as a third phone link feeding the same cloud as the ActiveTab/PC lanes; a three-way rail toggle labeled iPad/ActiveTab/PC; an auto-finder that auto-connects and auto-switches to the strongest connection; buglog pull + open-ticket fixes; reliable plug-and-play call audio with a record button into the conversation parser.
- **Hard constraint restated (unchanged verdict, apps/ipad-phone-bridge/README.md)**: iPadOS public APIs cannot hold the phone's Bluetooth Classic MAP/PBAP/HFP link (no RFCOMM; CoreBluetooth is GATT-only). The iPad lane is therefore a *feeder*: the bridge app proxies whichever BT-capable host holds the phone and now also feeds the SAME Firestore relay. Direct-BT stays behind the existing probe gate.
- **iPad third lane**: new `RelayPresenceService.swift` in `apps/ipad-phone-bridge` — writes `hosts.ios` presence into `phone-relay/owner` every 20 s (t/connected/quality, same shape as the other hosts) and, while the rail toggle prefers the iPad, pushes the full LAN-proxied state blob (`status/messages/calls/contacts`, DeskPhone's exact shape) into `phone-relay/state`. Registered in the Xcode project; wired through `BridgeModel`/`BridgeController` (`/status` now reports a `cloudRelay` block). Needs a Mac/Xcode build + install on the iPad.
- **Three-way toggle (iPad/ActiveTab/PC) + Auto chip**: `HOST_LABEL` is now `{ios: iPad, android: ActiveTab, windows: PC}` (ActiveTab = the Galaxy Tab Active) — one wording source, so every status line renames consistently. The rail control (AppSuiteChrome) is a 3-segment `md-outlined-segmented-button-set` plus an `md-filter-chip` "Auto"; manual picks pin a lane, Auto hands arbitration to the hosts. Collapsed rail icon cycles auto→ActiveTab→PC→iPad. `preferred` now takes `tablet|pc|ipad|auto` end-to-end (web `phone-host-control.js`, `RelayService.cs`, `RelayClient.kt`); old hosts read unknown values as `tablet` — safe rollout.
- **Auto-finder (strongest link wins)**: new presence map `phone-relay/owner.hosts.{id} = {t, connected, quality}` — every host beacons every arbitration tick, parked or not (`WritePresenceAsync` / `writePresence`, field-masked so hosts never clobber each other). One shared scoring spec lives in `phone-link.js` (`scoreHostLink`/`chooseAutoHost`: fresh-presence gate, +100 for a live BT link, quality 0–100, static tablet>PC tiebreak, 25-point hysteresis margin so healthy links never flap) and is ported verbatim into both native hosts, which use it to decide `shouldHold` when preferred is `auto`/`ipad`. Web `derivePhoneLinkState` gained the `auto` mode (no false "switching"; wording appends "(auto)").
- **Live call feed + record → parser**: new `call-audio-feed.js` — probes DeskPhone's loopback `call-audio/state` and turns the host's existing 16 kHz PCM WebSocket downlink into a plain MediaStream (sequential AudioBuffer scheduling). ConvCapture (the record→transcribe→AI-parse pipeline behind every record button) now offers **"Record live call feed"** as the primary action when the bridge answers — one click, no screen-share dialog, no device fiddling — with screen-share kept as the fallback; new `pclink` source labels ride the transcription pen. Plug-and-play host side: `CallAudioBridgeService` auto-detects carkit/speakerphone-class endpoints by name when the configured device is missing (`ResolveDownlinkDeviceId`/`ResolveUplinkDeviceId`) instead of silently defaulting to the PC's own mic.
- **Buglog**: no live pull possible from this sandbox — the admin key is PC-only and Firestore rules correctly deny unauthenticated reads (verified: anonymous auth disabled, REST reads 403). Durable fix shipped instead: the deployed MCP server (`apps/web/functions/mcp.js`) gained `list_bugs` / `add_bug_note` / `set_bug_status` (resolve requires a note — same contract as tools/bug-log-reader), so any Claude session with the MCP token can autopull and work tickets. Documented open holdouts worked: the Chief page's two `<select>`s are now real `md-outlined-select` (OutlinedSelect/SelectOption added to the shared m3.jsx layer).
- Verified: `npm run build` green in `apps/web`; `npm run test:phone` 36/36 (9 new: preferredHostId mapping, iPad/auto derive states, presence scoring, auto chooser stickiness/failover/margin/ignores-iPad). C#/Kotlin/Swift changes are code-reviewed but NOT compiled here (no dotnet/Android SDK/Xcode in the sandbox) — see the PC handoff addendum in `docs/ops/PC_HANDOFF_PHONELINK.md`.
- Version 4.43.132 → 4.44.132 (feat).

## 2026-07-14 Ticket Batch — Research Fallback, Two-Button Dictation, Column Expand, Retirements, Car-Kit Lane (web 4.45.132)

- Session scope: the 13 open 7/7–7/13 tickets + "build a car kit simulator". Sandbox session (no Firebase creds, no devices): verification = build gates, `npm run test:phone` 36/36, live-endpoint probes.
- **Shailos research "no results" (tickets 7/13 2:09 PM + 1:42 AM) — root cause found by probing production**: `/api/google-search` returns 503 "Google Custom Search is not configured" — the `GOOGLE_SEARCH_API_KEY`/`GOOGLE_SEARCH_CSE_ID` GitHub secrets are unset, every query silently became `[]`, and the UI lied "No search results found." Fixes: (1) `functions/google-search.js` now falls back to Sefaria's keyless public search API (`search-wrapper`, with a slim-down retry because Sefaria phrase-matches long queries) whenever CSE is unconfigured/over-quota/erroring — research works out of the box, $0; (2) `shailos-ai.js` `performResearch` keeps search errors and surfaces the real failure instead of "no results"; `runAiJob` errors now carry the HTTP status (the old bare "AI gateway error"). Owner TODO for full web coverage: add the two GitHub secrets (see `docs/ops/AI_CLOUD_INSPECTION_2026-07.md` §5).
- **Shaila dictation redesign (ticket 2:08 PM)**: all five per-field mini-mics (date/synopsis/asker/answer/raw) + the paste-modal 30-second dictate mic are GONE. Exactly two record controls remain — in the Shaila-question box and the Answer box: an unmistakable red ● that flips to a filled ■ + live "REC 0:12" counter (explicit start/stop toggle, 10-min safety cap, no auto-timeout, no ambiguous glow), then transcribes through the existing yeshivish protocol (`transcribe.yeshivish.v1`) and appends to the box. The top-bar Stop button lost its pulsing-glow animation and gained a live timer. Also fixed in passing: the NerveCenter "Record shaila" action (`action="record-shaila"`) had been a no-op since the native port — it now starts the recorder.
- **Unused ShailaTracker screen (ticket 1:17 AM)**: the full-screen "Shaila Transcriber" overlay in App.jsx was unreachable dead code — `setShowShailos(true)` exists nowhere. Deleted (overlay + state). The daily-use surface (rail → Shailos view → SuiteShailosPanel) is untouched; its Backup/Restore/Sync-check actions remain available elsewhere in App.jsx.
- **More Actions retired (ticket 1:17 AM)**: rail "More Actions" button removed (AppSuiteChrome), all five drawer triggers + all three drawer copies removed from NerveCenterNext, `actionCategories` plumbing dropped. Legacy `?ui=legacy` panel untouched.
- **Emails stacked per account (ticket 12:53 AM)**: `functions/google-workspace.js` summary merge flat-mapped account A then account B and capped at 30 — rabbidanziger filled the list and the second account only appeared after scrolling (or never). Merged list is now sorted by `internalDate` (Date-header fallback) before the cap — one interleaved newest-first inbox.
- **Column-layout expand/squish (ticket 3:33 AM)**: the card-grid's expand-to-page gesture (previously rows orientation only) now works in the wide 5-column orientation too — tapping a column header expands that column (`minmax(0,1fr)`) and squishes the other four to slim live strips (`minmax(120px,0.28fr)`); tap again to restore. MobileBox's sticky header gained the expand affordance + a trailing open_in_new for the full surface.
- **Answered-call-logged-as-Missed + phantom active call (tickets 6:28 PM 7/10, 5:06 PM 7/12 — HOST half, needs rebuilds)**: root cause in BOTH hosts' HFP CIEV handling: some AGs send `callsetup=0` BEFORE `call=1` when a call is answered on the handset, so the host filed a real conversation as Missed the instant setup ended (and the direction stuck). Android `HfpClient.kt` + Windows `HfpService.cs` now (1) wait 800 ms before declaring Missed (any call-state change cancels the verdict — Windows uses the existing `_callStateRevision`), (2) correct Missed→Incoming when `call=1` lands. Android additionally polls `AT+CLCC` after 60 s of in-call URC silence and clears a phantom "active call" when the phone reports no calls (the "screen still shows active call" half; web-side optimistic suppression shipped in 4.42.131). NOT compiled here (no SDK) — next tablet APK + DeskPhone build.
- **Car-kit simulator (owner ask)**: new `bt/CarKitClient.kt` — the real Android-car-kit path (`BluetoothHeadsetClient`, profile 16, stack-owned SCO call audio through the tablet's speaker/mic) via reflection with an honest probe (consumer builds often disable the profile; the API reports `supported:false` + reason instead of a dead toggle). HostService: persisted `carKitMode`, mutual exclusion with the raw RFCOMM lane, shared call-state consumer, `GET /carkit`, `POST /carkit/mode?on=`, `POST /carkit/audio?on=`; `/status` carries a `carKit` block. Full design/constraints/test steps: `docs/ops/CARKIT_SIM.md`. Needs the tablet APK rebuild.
- **Tickets already resolved by 4.37–4.44 (verified present on main, pending owner device confirmation)**: handover wording ("waiting to confirm handover" grace state, 5:06 PM), history wipe on handoff (`mergeMessageFeeds`/`mergeCallFeeds`, 1:37 PM), toggle/status overhaul (10:18 PM 7/9), three-lane + auto-finder (11:31 AM 7/7).
...
## 2026-07-27 Five-Ticket Sweep + A Cloud Session That Can Finally Read Firestore (web 4.108.3 → 4.110.0)

- Session scope: the five open tickets (pasted by the owner), plus two standing asks — make the repo private, and give cloud sessions real Firebase access "whatever works best, do it all yourself backend".
- **Cloud → Firestore bridge (`tools/firestore-bridge/`, `.github/workflows/firestore-bridge.yml`)**: the recurring "no live pull possible from this sandbox" note in this log is now false. A cloud session has no Firebase credential of any kind (admin key is PC-only, rules correctly deny unauthenticated reads, the deployed `/mcp` endpoint wants a bearer token the session cannot see), but `MCP_READ_TOKEN` exists as a repo secret and Actions can read it. The session generates a throwaway RSA keypair, the runner makes the MCP call and returns the answer AES-256-GCM encrypted under it (RSA-OAEP-256 wrapped key). Actions logs are world-readable and this repo is still public, so the encryption is what makes the bridge safe — the Bug Log never lands in a readable log. Two ways in, because dispatching needs `actions:write` and this session's GitHub App does not hold it (`actions_run_trigger` → 403): a `workflow_dispatch`, or a push of `.bridge-request.json` to a `bridge/**` branch. The runner commits the encrypted answer back to that branch, so the session fetches and decrypts locally.
- Exercised end to end this session: pulled all five open tickets, then closed each with a resolution note. Verified afterwards through the same bridge — unresolved count 0, five resolved tickets each carrying one note.
- **Tapping an email crashed the whole app (ticket oINP7RIJ)**: `mailReaderPanel` is built during render and 4.105.0 placed it ~120 lines above both `C` and `decodeSnippet`, which it calls — const bindings do not hoist, so the first tap on any message threw ReferenceError out of render into `AppErrorBoundary` ("Something went wrong. Tap to reload."). In every layout, since it shipped; it survived three releases because a cached full body skips the crashing line, so a second tap looked fine. Block moved below its dependencies. The card grid also never rendered the reader at all — added.
- **Tablet card expansion (same ticket)**: siblings sized `min-content` and never collapsed, so a busy sibling took the height and the expanded card shrank. Measured: expanding Mail took Mail 211px → 0 while Calendar grew 219px. Siblings are now fixed header strips (56px / 96px when the header wraps), expanded card takes the remainder via `1fr`. Verified per card: tapped card +179px, siblings at the strip.
- **Card bodies would not scroll (same ticket)**: `scrollable ? "auto" : "hidden"` assumed `useFitRows` renders exactly what fits. The "+N more" chip is appended after the fitted rows and was never part of the fit — Mail's last row ended 29px below the body and the chip 63px below that. Bodies always scroll now.
- **Ragged column tops (ticket fuRD6zZp)**: only Mail and Calendar carry header actions, and `narrowActions` wrapped them onto a second line. Measured at 1920px: 154px headers against 56px, content tops 284 against 186. One row, pinned 56px, all tops at 186; a narrow column folds the card's actions behind one uniform overflow button.
- **Phone-log search (ticket 7t2muAb0)**: added to both surfaces that show the log — NerveCenter phone surface and the DeskPhone web Calls tab. Searches the full history rather than the 30-40-entry feed cap (and lifts the cap while querying), searches every message in a thread, matches names/numbers/digits/kind with all-terms-any-order. The DeskPhone half uses real M3 components themed to the dp palette: that file predates the GM3 rule and is full of raw markup, but the ratchet correctly caught the two new raw elements (+2) and they were converted rather than re-baselined.
- **Recording pen on screen (ticket WUuXgang)**: Settings → Recordings, present whether the pen is full or empty; previously the only entrance was a floating button that renders only while something is held, so an empty pen removed the feature from the app. Rows lead with a terse AI one-liner (same deployed job the Bug Log uses, cached in both pens) plus a date stamp. **Correction recorded on the ticket**: retention is 10 days, not 20 — `PEN_RETENTION_MS`, per the 2026-07-19 spec. Not changed; the Settings tab now prints the real number.
- **Buglog tap-outside (ticket xFuKmpve)**: closes on outside tap and Escape, via a capture-phase document listener rather than the mail card's full-viewport catcher — the catcher would have blocked scroll and hover on the page this panel exists to be used over.
