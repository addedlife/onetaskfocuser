...
- Gates: `npm run build` (apps/web) → 0 errors. BUILD-VERIFIED; tickets closed via reader-tool `resolve` with process notes.
...
- Android host reliability fixes (`apps/phone-host-android`): `map.onStatus` was never wired — `/status` reported MAP "Idle" forever; MNS server was created only at service start so a null adapter (Bluetooth off at boot) killed live push permanently — now created lazily at connect; `startForeground` guarded (connectedDevice FGS without BLUETOOTH_CONNECT crashes on Android 12+) and BootReceiver/MainActivity no longer start the service unpermissioned; every scheduled task body wrapped in `runCatching` (one throw permanently cancels a ScheduledExecutorService task — would have silently killed the watchdog); store saves now atomic (tmp+rename, dirty flag restored on failure) so power loss can't truncate messages.json; sent-echo dedupe in `MessageStore.merge` (send racing the sync produced a duplicate bubble); `HfpClient.connect` closes its socket on handshake failure (leaked RFCOMM channel blocked reconnects); `onDestroy` Bluetooth teardown moved off the main thread (OBEX round-trips under the sync lock → ANR).
- iPad bridge reliability fixes (`apps/ipad-phone-bridge`): proxy work moved off the NWListener serial queue to a concurrent queue — the web surface fires /status /messages /calls /contacts in parallel with 2.5 s timeouts and the serial queue would time three out (primary-lane bug); ContentView now observes `probe`/`lanHost` directly (nested ObservableObjects don't propagate through the parent — the status card never refreshed).
- Consumer UI rewrite, both platforms: Android `MainActivity` → plain-language status card (Connected to <phone name> / Connecting… / Choose your phone / Paused / permission prompt), one contextual action button, device picker by name with ✓ (no MAC addresses), "Open Bluetooth settings" affordance, all technical detail (addresses, API URLs, protocol log) collapsed behind "Advanced". iPad `ContentView` → "Phone Link" title, single status card ("Connected — calls and texts from your phone are available on this iPad"), entire Bluetooth probe/diagnostics moved into a collapsed Advanced DisclosureGroup; app display name "WebPhone Bridge" → "Shamash Phone Link".
- Gates: `gradle :app:assembleDebug` → BUILD SUCCESSFUL after all changes. Swift remains source-only (no Xcode on Linux). Runtime smoke with the real phone still owed (unchanged).

## 2026-07-05 Local BT phone hosts — Android host app built; iPad becomes LAN proxy; relay-exit path defined

- Owner request: local Bluetooth phone host for iOS and Android ("starting from iOS then android, seamless switching, then ditch the relay"). Branch `claude/phonehost-bluetooth-local-aby9rp` (not pushed to main — native apps, no web release).
- iOS-first is blocked by platform, not by code: MAP/PBAP/HFP are Bluetooth Classic RFCOMM/SDP profiles, NOT GATT services — Apple's public CoreBluetooth can never surface them, and iPadOS has no public RFCOMM API (ExternalAccessory = MFi hardware only). The existing `apps/ipad-phone-bridge` probe is exactly this decision gate; verdict recorded in its README. Standalone-iPad would require MFi/hardware-bridge work.
- **NEW `apps/phone-host-android`** — the feasible local host, built first: Kotlin app (zero external deps, AGP 8.7.3/Kotlin 2.0.21, minSdk 26) porting the field-proven Windows stack: `ObexClient.kt` (OBEX 1.0 CONNECT, Fig 52/MediaTek header-offset + no-empty-EndBody quirks), `MapClient.kt` (delta sync w/ seeded handles + MaxListCount=1 probes, paginated full-history loader, bMessage/MMS-MIME parser, send w/ SMS_GSM→CDMA→MMS ladder, read/delete pushback, MNS registration), `MnsServer.kt` (RFCOMM 0x1133 OBEX server), `PbapClient.kt` (ich/och/mch call logs + NEW pb.vcf contacts import), `HfpClient.kt` (BRSF=4 — deliberately outside the audio path like Windows; RING/+CLIP/+CIEV state machine, ATA/AT+CHUP/ATD control, 30s keepalive probe), JSON-file stores, and `LocalApiServer.kt` on 0.0.0.0:8765 with the Windows ControlApiService contract (same /status /messages /calls /contacts shapes from MainViewModel, same CORS+PNA headers). Foreground service + boot receiver + reconnect watchdog; advertises `_shamash-phonehost._tcp` via NSD.
- **Seamless switching**: one BT client at a time is a profile-level fact, so hosts hand the link over — `POST /handoff-release` parks a host (clean HFP/MAP/MNS drop, watchdog off), `POST /connect` on another host takes the link; per-host stores + seeded delta sync make the swap cheap.
- **iPad lane (`apps/ipad-phone-bridge`)**: new `LanHostClient.swift` — Bonjour-discovers the active host (manual `POST /lan-host` fallback) and raw-proxies the entire host contract on the iPad's own 127.0.0.1:8765, so the web app works unchanged with NO cloud relay (native URLSession sidesteps the https→LAN-IP mixed-content block; loopback is exempt for the web app). Probe endpoints stay local as the gate. pbxproj updated (new file + merged Info.plist with `NSBonjourServices`).
- **Call audio limitation (Android host)**: HFP SCO audio is not app-accessible on Android (no BthHFEnum equivalent) — v1 is call control/state only; voice stays on the phone. Documented in the app README.
- Relay exit path: PC and tablet hosts serve their own browser via loopback; iPad consumes whichever host is live via the bridge proxy. Once each device's lane is runtime-verified, `phoneRelay` (§15.5) can be decommissioned.
- Gates: `gradle :app:assembleDebug` → BUILD SUCCESSFUL, `app-debug.apk` produced (Linux CI-like env, SDK 34). Swift lane is source-only (no Xcode on Linux — same precedent as 2026-05-18). NOT runtime-verified against the real phone yet: first on-device smoke (pair→connect→sync→send→call state, then handoff between hosts) is the next session's gate.

## 2026-07-03 DeskPhone b326 + RTDB push — commands by true stream, zero idle reads (4.34.114)

- Owner GO on replacing the b324 poll with real push. New **Realtime Database** instance `onetaskonly-app-default-rtdb` (us-central1) provisioned via the Management API; rules deployed opening ONLY `phone-relay/commands` (same posture as the old Firestore doc — queuing still gated behind Firebase sign-in in the function). `firebase.json` gains the `database` block; rules file `apps/web/database.rules.json`.
- Host b326 (`7b43799`): `StreamRtdbCommandsAsync` holds an SSE stream on the commands node (RTDB REST genuinely streams, unlike Firestore REST listen); put/patch events trigger `DrainMailboxOnceAsync` (GET → PUT null clear-before-dispatch → dispatch; `NormalizeRtdbArray` handles RTDB's object-map arrays). The poll remains as safety net: 120s while the stream is healthy, 2s/6s adaptive when down; plus a 60s legacy sweep of the old Firestore commands doc (remove in a future build). Function (`b268b39`): command/push/drain paths moved to the RTDB node.
- Cost: idle Firestore reads from the command channel → **zero** (was 43k/day originally, 14k/day on b324). RTDB usage rounds to $0.
- VERIFIED: command PUT to the RTDB node → host log `[RELAY CMD] /refresh` in **420 ms**. b326 accepted and running; deploy Action for the function green.
- RETRACTION: the previous entry's "+4h host time skew" claim is doubtful — the session spanned many hours and the comparison used a stale local-time assumption; host log clock matches system clock when compared live. Re-verify against a known call before treating call timestamps as a bug.
- Consolidation audit (same session, ydanziger20@gmail.com): 9 GCP projects inventoried; keys/APIs swept. Load-bearing: `onetaskonly-app` (everything), `shamashpro4-health-sync` (project number 684600201717 = the Google Workspace OAuth client), `gen-lang-client-0743523606` (Gemini keys "Onetask App AI Functions", "Message analyzer"), `gen-lang-client-0892770796` (overflow Gemini key). Caution: `primordial-port-459016-s1` and `danziger-kiosk-2026` both have androidmanagement.googleapis.com enabled (possible managed-device policies — deleting could brick a kiosk enrollment). Likely junk: `crack-walker-459016-i7` (console cruft only), `rabbis-metrics` (dup of rabbi-s-metrics, telemetry only). NOTHING deleted — report delivered to owner; second Google profile not credentialed on this machine, not yet inventoried.
...
- Committed the pre-existing uncommitted Bug Log feature as its own checkpoint (`f88ec4b`, feat) — floating Firestore-backed bug/idea widget (`users/{uid}/bugs`), Store.addBug/updateBug/deleteBug + BugLog.jsx wired into App on every screen.
...
- Version bumped 4.30.106 → 4.32.106 (two feats: BugLog + rebuild). Pushing to `origin/main` also ships the BugLog widget live (ancestor commit).

## 2026-07-06 Bug-Log Triage Pass — 22 Tickets Closed Out (web 4.34.115)

- Fixed the bug-log reader tool (`tools/bug-log-reader`): it queried Firestore by the Firebase Auth uid, but the app keys user data by email prefix (canonicalUid) and the owner uses the rabbi account — bugs live at `users/rabbidanziger/bugs`. Reader now targets that path and gained `note <id> <text>` and `status <id> <status>` write commands, satisfying the "connect the buglog for autopull" and "edit/update function" tickets.
- Queue list container, empty state, and four insights cards in `App.jsx` used `borderRadius:RADIUS.pill` with `overflow:hidden` — the "task queue is a big pill with top/bottom cut off" ticket. All switched to `RADIUS.md`.
- The full-width `md-list` "Queue · N" shortcut row on the tasks main page replaced with a centered `md-assist-chip` (the "queue view bar too long, was a pill" ticket). Verified visually in dev.
- NerveCenter task-add controls (both NerveCenterPanel and NerveCenterNext): `compactAddDot` dropped its tinted pill background/border; icon changed from `add` to `check` — bare colored checkmarks per the owner's ticket. Verified visually in dev.
...
- Verified live against host b326 (`/?standalone=deskphone`): status line reads "Your phone is out of range — reconnects when nearby", no MAC addresses anywhere, single reconnect arrow wired to `/connect` — the phone-status "gibberish"/reconnect/justification tickets were fixed by 4.33.113/114 + b324–b326 and confirmed on-screen.
- `npm run build` passed in `apps/web`; version 4.34.114 → 4.34.115 (fix).

## 2026-07-10 Phone-Connection Ticket Batch — Sends, Doubles, Toggle, Status Truth (web 4.36.125)

- Session context: Claude-on-web session (no Firebase credentials, no live devices), working the five open 7/7–7/9 phone tickets on branch `claude/shamash-phone-connection-bugs-nc0cz6`. Verification = build + module unit tests + headless-Chromium shell smoke; on-device confirmation still needed (see "pending" below).
- **Instant sends everywhere (ticket: "text freezes 5 seconds… do it immediately in all sms sending fields")**: new shared echo store `apps/web/src/08-app-split/utils/pending-sms.js` — every composer (NerveCenter phone surface compose/reply, DeskPhone web reply, new-message tab, retry buttons) now drops the typed text into the thread instantly as a "Sending…" bubble, clears the box, and runs the command in the background (`post`/`runCommand` gained a background mode that skips the busy-freeze and the global error banner). Failure flips the bubble to "Failed" + Retry, so nothing typed is ever lost; success leaves a plain sent bubble until the host's own copy arrives and the echo auto-drops (match = normalized recipient + whitespace-tolerant body + 10-min skew window).
- **Send-doubling (ticket: "one says sent the other forever on confirming on phone")**, three layers:
  1. Windows host `MessageStoreService.Merge`: the local-bubble ↔ phone-sent-copy reconcile window was a flat 90 s of PC-clock-vs-phone-clock delta — ordinary clock skew beat it and the "Confirming" bubble never adopted the phone copy. Now 15 min while the local bubble still carries an in-flight SendStatus (only our own recent send can look like that), 90 s otherwise. **Needs a PC host rebuild (next b3xx) to take effect.**
  ...
  3. Web (`collapseHostDoubles`, live immediately): both thread builders visually collapse an outgoing bubble stuck on Sending/Confirming when a confirmed copy of the same text exists in the same thread within 10 min — so existing stuck doubles in old host blobs disappear from every browser today.
- **Rail host toggle (ticket: "huge and ugly and not gm3… idk if it even works")**: still the real `md-switch` (M3 rule) but sized to rail density via official md-switch tokens (40×22 track, 30px state layer), `flexShrink:0` so it can't squish, theme-bridged unselected track/outline. Label now shows the host that ACTUALLY holds the phone (owner-doc heartbeat) and reads "Handing to PC…/Tablet…" (italic) while the flip hasn't landed — the toggle visibly does something the moment it's tapped, and honestly shows when the hosts haven't obeyed yet.
- **Status truth (tickets: "says can't connect but shows recent calls from 4 hours ago", "no status indicator")**: when the link is down but old data is on screen the one status line now says "Offline — showing texts & calls from 4h ago"; transient refresh failures with data on screen no longer paint the red "can't reach" banner over a live-looking feed (hard failures and actionable states — sign-in, rules, no host — still do). Connected line appends "handing to X…" during a host handoff.
- Verified: `npm run build` green (2×); `node` unit run of pending-sms (echo add/match/reconcile/expiry, phone/body normalization, double-collapse keeps confirmed + unrelated, drops stuck double) — 9/9 ok; built bundle served via `vite preview` + headless Chromium: suite shell, rail (compact switch, "Phone: Tablet" label), and phone card ("Sign in to see your phone here." auth state) all render with zero page JS errors (network errors expected — sandbox blocks Firebase/fonts).
- Pending on-device: host rebuilds (PC + tablet) for the root-cause reconcile fixes; live relay send from iPad/desktop to confirm echo → host-copy handoff timing; visual pass on the resized switch in all 8 themes.
- Version 4.36.124 → 4.36.125 (fix).

## 2026-07-10 Phone-Link Overhaul — One Truth Machine, Exact Send IDs, Never-Blank (web 4.37.125)

- Follow-through on the second-opinion review of the 4.36.125 ticket batch: replace compensations with architecture. Device roles confirmed by owner (phone = no apps possible → BT host mandatory; Galaxy tablet = primary host; PC = standby + call audio; iPad = web consumer), so the tablet-primary arbitration stays — no from-scratch rebuild.
...
- **One status truth**: new pure module `apps/web/src/08-app-split/phone-link.js` — single state machine (`connected / switching / offline / connecting / no-host` + data age) and single wording source (`describePhoneLink`). Both surfaces now derive every dot/label from it; the DeskPhone page's separate 45 s window is gone (it also now subscribes to the owner heartbeat doc), so two screens can no longer disagree about liveness. `phone-host-control.js` re-exports the constants from it (one source).
- **Never-blank rule**: neither surface wipes messages/calls/contacts on fetch failures anymore (NerveCenter used to blank after 3 bad cycles; DeskPhone page on ANY error). Old data stays with "Offline — showing texts & calls from Xh ago"; loopback-path liveness now keyed to last-successful-fetch age so a dead local host can't show a stuck green light.
- **Exact send identity (cid)**: every composer's echo id now rides `/send?...&cid=` (and the attachments JSON) → both hosts stamp it as the message's own id (Windows `LocalId`, preserved across phone-copy adoption; Android `localId`) → the state blob returns it → `pending-sms.js` reconciles echo↔host-copy by exact id. Fuzzy recipient+body+time matching demoted to fallback for pre-rebuild hosts. Windows delegates changed: `Send(to, body, cid?)`, `SendWithAttachments(to, body, attachments, cid?)` in ControlApiService + RelayService + MainViewModel.
- **Diagnosis + tests + cleanup**: `?phonediag=1` overlay on the phone surface shows the raw machine (state, owner doc, heartbeat ages, echo list) for minutes-not-guesswork triage; committed `npm run test:phone` suite (node:test, 19 tests — machine states, wording, signature repaint, cid + fuzzy reconcile, double-collapse) — all pass; dead `DeskPhoneMiniDock.jsx` deleted.
...
- Pending on PC (see `docs/ops/PC_HANDOFF_PHONELINK.md` — step-by-step): rebuild DeskPhone (b329) + tablet APK to activate cid + reconcile fixes; then a follow-up session deletes the legacy fallbacks listed in that doc §5.
- Version 4.36.125 → 4.37.125 (feat).
