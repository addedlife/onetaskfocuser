...
Reviewed slice: all 199 native action rows have now been classified against `src/10-deskphone-web.jsx`: 193 browser-implemented rows, 0 host/API-needed rows, and 6 native-only WPF/template rows. Covered browser behavior includes navigation rail collapse, phone/settings navigation, reconnect/settings rail actions, reconnect prompt Choose Device opening browser Settings, build prompt accept/snooze/show controls through host endpoints, active-call mute through a host endpoint, call banner answer/hangup actions, browser full New Message compose/cancel/contact-pick/send, browser message Forward into a prefilled New Message draft, browser message Pin through a host endpoint, browser pinned-message strip jump behavior, browser message Delete through a host endpoint, browser message-delete Undo through a host endpoint, browser contacts list/detail with browser new/text/call/edit/save/delete behavior, contacts/developer/log shell routing, top/header new-message entrypoints, conversation filters, unread-first sorting, row-menu and thread-header block/pin/mute/read/unread actions through direct host endpoints, browser add/edit contact routing, show/hide thread list, thread search previous/next navigation, thread clear-search, call actions through `/dial`, call-history All/Missed/In/Out filters, full Calls navigation backed by all native `/calls` data, Make Call opening the browser dialer, full Calls hide/show recents, call-row message/call/browser-delete/delete-all plus block through direct host endpoints, thread-side dialer show/hide/backspace/keypad/text/call/voicemail controls including typed-number browser compose rows, settings section navigation for Appearance/Contact Sync/Audio, settings saved/scanned device scan/connect/default/forget controls, settings host-tool actions for Bluetooth Settings, Sound Settings, audio refresh, builds folder, event log, appearance reset, theme-sync refresh/toggle, history background fetching, dark mode, VCF/contact import, ignore pending contacts, contact-sync folder, message backup export, live log, clear log, and UI auditor, duplicate native Calls-tab dialpad rows, attachment save where browser data is available, thread/full-compose attachment add/remove and send through `/send-with-attachments`, scroll-to-latest, and text send through `/send`. No host/API-needed rows remain; the only non-browser rows are the 6 native-only WPF/template rows.
