...
- **Research**: three-step — (1) Gemini converts shaila → clean Google query, (2) Serper.dev returns real article URLs (PDFs filtered out), (3) Gemini summarizes + extracts seforim + picks 1-3 word key term per article for Text Fragment scroll (`#:~:text=term`). Requires `SERPER_API_KEY` Netlify env var.
