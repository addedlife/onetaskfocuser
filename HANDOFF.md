...
- **DeskPhone web screen** (`src/10-deskphone-web.jsx`): active exact-parity surface. Current reviewed ledger: 75 of 199 native action rows reviewed (35 implemented-web, 40 host-api-needed, 124 not-yet-reviewed). Latest slices added browser All/Missed/In/Out filters, call-row message/call/block/delete controls, and a compact thread-side dialer with show/hide/backspace/call/voicemail actions. `artifacts/deskphone-web-parity-smoke.cjs` covers the verified browser and handoff behavior. Continue in small inventory-backed slices; preserve native handoff shortcuts until host endpoints exist.
...
Latest: DeskPhone Web parity slice adds thread-side dialer actions and updates the parity ledger.

```
(pending push) feat: DeskPhone web thread-side dialer parity
