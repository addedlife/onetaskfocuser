...
- **DeskPhone web screen** (`src/10-deskphone-web.jsx`): active exact-parity surface. Current reviewed ledger: 199 of 199 native action rows reviewed (114 implemented-web, 79 host-api-needed, 6 native-only, 0 not-yet-reviewed). Latest slices added browser All/Missed/In/Out filters, call-row message/call/block/delete controls, compact thread-side dialer controls including keypad/Text handoff, in-browser thread search Previous/Next, exact-source row-menu and thread-header block/pin/mute/read/unread handoffs including duplicate native header rows, shell/reconnect/build/call-banner/contact-import/contact-action/Contacts-tab/full-compose/settings/log/collapsed-calls parity mapping, and Settings host-tool buttons for Bluetooth Settings, Sound Settings, audio refresh, builds folder, and event log. `artifacts/deskphone-web-parity-smoke.cjs` covers the verified browser and handoff behavior. Next runtime work should keep converting `host-api-needed` rows into host/browser endpoints.
...
43826e7 docs: map DeskPhone web settings device controls
765f467 docs: map DeskPhone web contact editor and settings rows
4fa083c docs: map DeskPhone web duplicate Calls dialpad rows
47e7557 docs: map DeskPhone web full-compose blockers
25b3747 feat: DeskPhone web thread-side dialpad keys
0beb4d3 docs: map DeskPhone web log and collapsed call controls
baa9f8d docs: map DeskPhone web Contacts tab actions
6b9f315 docs: map DeskPhone web duplicate dialer controls
017b160 docs: map DeskPhone web duplicate call controls
0b8d455 docs: map DeskPhone web alternate call row controls
db4433f docs: map DeskPhone web alternate call history controls
fc3933d docs: map DeskPhone web contact action blockers
143b829 docs: map DeskPhone web shell destination parity
ad97cba feat: DeskPhone web dialer text handoff
0350b0e docs: map DeskPhone web build and call controls
50615fe docs: map DeskPhone web reconnect parity controls
