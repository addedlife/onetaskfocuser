...
- **DeskPhone web screen** (`src/10-deskphone-web.jsx`): active exact-parity surface. Current reviewed ledger: 70 of 199 native action rows reviewed (30 implemented-web, 40 host-api-needed, 129 not-yet-reviewed). Latest slices added browser All/Missed/In/Out filters plus call-row message/call/block/delete controls to the thread-side call-history column, with `artifacts/deskphone-web-parity-smoke.cjs` covering the verified browser and handoff behavior. Continue in small inventory-backed slices; preserve native handoff shortcuts until host endpoints exist.
...
Latest: DeskPhone Web parity slice adds call-row actions and updates the parity ledger.

```
(pending push) feat: DeskPhone web call-row parity actions
