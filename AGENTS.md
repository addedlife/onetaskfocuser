...
- Commit and push verified web changes when requested; deploy to production only after explicit approval in the current thread.
...
- Netlify production should be deployed from `apps/web` with `npx netlify deploy --prod` only after explicit approval in the current thread.
